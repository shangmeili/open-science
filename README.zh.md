...
中的双策略、三状态队列案例只有在研究者点击后才安装。它是一套完整的虚构教学流程，
包括研究问题、参数假设登记与证据缺口、概念模型、基线分析、8个参数的单因素敏感性
分析、3个结构情景、固定随机种子的1000次概率敏感性分析、机械检查、报告草稿和研究者
复核清单。不依赖第三方包的 `run_analysis.py` 会绑定脚本、分析设定和CSV的准确哈希，
并逐字节复算 `expected/base-case-result.json`。研究者点击本地运行后，桌面应用可在没有
设置模型的情况下写出 `outputs/complete-case-result.json`、`outputs/teaching-report.md`
及两个兼容性敏感性结果，同时保留运行和溯源记录；案例内容不会发送给模型服务。
其中的数值只是教学假设，不是临床或经济学证据，也不能生成科学批准、具有成本效果、
报销或政策结论。
...
项目是正在积极开发、可以安装运行的桌面产品。最可靠的当前实现日志是 [`PROGRESS.md`](./PROGRESS.md)。
...
1.0.0 将“新建任务”做成不受模板限制的自然语言入口，四个药物经济学快捷项只填入草稿；“研究工作台”保留为
