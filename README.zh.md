...
当前验证的 0.1.35 本地 x64 macOS 构建尚未代码签名或 notarize。
...
与跨平台发布暂停，直到 macOS 路径通过验收。当前源码为 `0.1.36`；新的 Intel 安装包完成构建和验证前，
当前已验证的 x64 macOS 安装包仍为 `0.1.35`。0.1.36 增加了包含 25 份资料的版本化中文药物经济学学习库，
并提供明确的本机安装入口。应用会核对安装包中的清单和每份资料的 SHA-256，不调用模型、不联网，
...
七种语言的资源文件。0.1.32 建立的失败关闭项目约束和研究者科学决策权保持不变。80,139,562 字节的
`AI4HEOR_0.1.35_x64.dmg` 的 SHA-256 为
`11d1e4eb8924d1834003a78042b78e4befdce4b25ee2ecc1ca0253e449654003`，已从干净的受控源码提交
`ad9f139975cef9be943cb23b702fd79bdcfe0592` 构建。验证确认 286 个受控资源与源码逐字节一致，
177 项包内 HEOR 测试全部通过，并完成全新启动和旧工作区迁移检查。另一次隔离的真实界面验收
在未配置模型的情况下完成基线及两项敏感性分析，生成 1 条成功运行记录和 3 条溯源记录；随后修改固定输入时，
应用使用自然中文说明拒绝复算，原有结果文件未被覆盖。该包仍未经 Developer ID 签名或 notarize；
这些工程证据不代表科学有效性。
