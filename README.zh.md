...
与跨平台发布暂停，直到 macOS 路径通过验收。当前源码为 `0.1.43`；当前经过验证的 x64 macOS 安装包仍为
`0.1.41`。从 `b9f5b61` 构建的新 `AI4HEOR_0.1.42_x64.dmg` 候选包为
81,638,295 字节，SHA-256 是
`0c7d25ecda62a903c78ac4e14d4a64e3c8347bb21112ab16aaa22910d4425db9`。
只读核验已确认 x86-64 架构、0.1.42 身份、OpenCode 1.17.13、uv 0.11.26、
340 个受控资源和包内 177 项 HEOR 测试；由于已安装应用仍在运行，该精确候选包的
隔离首次启动尚未核验，因此暂不取代已完成验收交接的 0.1.41 包。0.1.43 新增第一方、仅本地运行的 `literature-review` Skill：它可导入、去重、校验和确定性导出带来源记录的 RIS、受控 BibTeX 和 CSL-JSON 文献数据，并保留字段冲突供研究者复核；不声称已实现 CSL 样式渲染或文献信息正确性。该版本的 Intel macOS 包仍待从干净的已跟踪源码构建和独立核验。0.1.42 增加了本机使用准备检查，核对项目文件夹、内置 Skill、药物经济学计算资源、
