...
与跨平台发布暂停，直到 macOS 路径通过验收。当前源码为 `0.1.51`；已完成隔离首次启动核验的 x64 macOS 交接包仍为
`0.1.41`。当前候选包 `AI4HEOR_0.1.50_x64.dmg` 从 `19ac87b` 构建，大小为
90,082,675 字节，SHA-256 是
...
独立只读核验已确认 x86-64 架构、0.1.50 身份、OpenCode 1.17.13、uv 0.11.26、
359 个受控资源和包内 177 项 HEOR 测试，并取代 0.1.49 成为当前候选包。由于已安装应用仍在运行，该精确候选包的
