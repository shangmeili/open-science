...
当前验证的 0.1.52 本地 x64 macOS 构建尚未代码签名或 notarize。
...
与跨平台发布暂停，直到 macOS 路径通过验收。当前源码为 `0.1.52`；已完成隔离首次启动核验的 x64 macOS
候选包也是 `0.1.52`。`AI4HEOR_0.1.52_x64.dmg` 从 `0b77504` 构建，大小为
90,047,182 字节，SHA-256 是
...
独立只读核验已确认 x86-64 架构、0.1.52 身份、OpenCode 1.17.13、uv 0.11.26、
359 个受控资源和包内 177 项 HEOR 测试，并取代 0.1.51 成为当前候选包。临时应用副本还在已安装应用保持运行时，
...
`AI4HEOR_0.1.52_x64.dmg` 仅用于 Intel Mac 内部测试。打开前先核对 SHA-256
`8f4e2feb2badc36db8372012169f68bbe867403386fc66cd5d0e89075a398138`。由于该包尚未
