...
| 项目与会话 | 项目是可在多个会话中共用资料和上下文的具名本地工作区。单独会话拥有自己的本地研究作用域，助手、Skill、HEOR 方法、文件、复核和审计能力不降级，只是不跨会话共用。 |
...
当前验证的 0.1.54 本地 x64 macOS 构建尚未代码签名或 notarize。
...
与跨平台发布暂停，直到 macOS 路径通过验收。当前源码为 `0.1.54`，已完成隔离首次启动核验的 x64 macOS
候选包也是 `0.1.54`。`AI4HEOR_0.1.54_x64.dmg` 大小为 90,080,697 字节，SHA-256 是
`2c8c70bd96fc9f78778f0c9653518b23de3d915c2198467c968345fd4b372aff`。
0.1.54 将项目和会话分开，但不限制单独会话。项目是可由多个会话共用资料和上下文的具名本地工作区；单独会话
拥有自己的本地研究作用域，助手、Skill、HEOR 方法、文件、复核和审计能力完全相同。全局“新建会话”用于开始
单独会话，项目旁的 `+` 用于在该项目内开始会话，侧栏单独列出两类会话。这一区分已覆盖七种界面语言。候选版本
已安装到 `/Applications`，并通过打包运行时流式测试及一次凭据隔离的 MiniMax-M3 真实请求。
...
独立只读核验已确认 x86-64 架构、0.1.54 身份、OpenCode 1.17.13、uv 0.11.26、
359 个受控资源和包内 177 项 HEOR 测试，并取代 0.1.53 成为当前候选包。临时应用副本还
...
`AI4HEOR_0.1.54_x64.dmg` 仅用于 Intel Mac 内部测试。打开前先核对 SHA-256
`2c8c70bd96fc9f78778f0c9653518b23de3d915c2198467c968345fd4b372aff`。由于该包尚未
