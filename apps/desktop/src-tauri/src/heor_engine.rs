...
fn workflow_status(
    ...
    let locally_authorized = plan_matches && reference_case_status != "draft";
    HeorWorkflowStatus {
        ...
        effective_approved_gates: log.effective_approved_gates,
        input_sha256,
...
pub fn run_heor_markov(
    ...
    Ok(HeorRunResult {
        workflow: workflow_status(approval_log, input_sha256, &reference_case_status),
        calculation,
    })
}
...
mod tests {
    ...
    fn authorization_requires_the_approved_analysis_plan_to_match_the_input() {
        ...
        let authorized = workflow_status(approved_log(&input_hash), input_hash.clone(), "current");
        assert_eq!(
        ...
        let changed = workflow_status(approved_log(&input_hash), "d".repeat(64), "current");
    ...
    fn draft_reference_case_cannot_be_locally_authorized() {
        let input_hash = "c".repeat(64);
        let status = workflow_status(approved_log(&input_hash), input_hash, "draft");
