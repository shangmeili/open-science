...
fn required_input_paths(plan: &serde_json::Value) -> Vec<String> {
    ...
    let structure_neutral = plan
        .get("schema_version")
        .and_then(serde_json::Value::as_str)
        .is_some_and(|version| matches!(version, "0.12.0" | "0.13.0" | "0.14.0" | "0.15.0"));
...
pub fn audit_plan(plan: &serde_json::Value) -> EvidenceAudit {
    ...
    if !matches!(
        ...
        invalid_mappings
            .push("schema_version must be 0.3.0 through 0.15.0 for approval review".into());
    }
        ...
        })
    {
            ...
            || actual != unique
        {
            invalid_mappings.push(
                "schema 0.8.0 through 0.15.0 requires 2-16 unique safe strategy ids, an exact strategies object, and baseline_strategy_id first".into(),
            );
        }
    }
    for role in declared_strategy_ids {
            ...
            && (has_matrix || has_schedule)
        {
            ...
            .get("schema_version")
            .and_then(serde_json::Value::as_str)
            .is_none_or(|version| !matches!(version, "0.12.0" | "0.13.0" | "0.14.0" | "0.15.0"))
            && has_matrix == has_schedule
        {
            invalid_mappings.push(format!(
...
mod tests {
    ...
    fn prior_schema_and_stale_derivation_snapshot_fail_closed() {
        ...
        assert!(errors.contains("schema_version must be 0.3.0 through 0.15.0"));
