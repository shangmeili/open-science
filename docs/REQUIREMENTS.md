...
- **Status.** 🟡 AI4HEOR 0.1.41 macOS x64 is locally verified from tracked commit
  `38e061708e56ee82d53ea3d68a79b95dd25ea5dc`. The current 0.1.49 x64 candidate from
  `d0da1afba38a0dcbf21cef4b938490a803e5be27` is structurally and byte-for-byte verified with all 177 packaged HEOR
...
| P1-4 | Windows + macOS installers | P1 | 🟡 Partial — 0.1.41 macOS x64 is fully locally verified; the 0.1.49 x64 candidate is structurally verified with exact first launch pending while the installed app remains open; 0.1.31 arm64 is cross-host content-inspected, and Linux x64 remains verified at 0.1.30; cross-platform work is paused for Intel macOS product acceptance; Windows/native-arm execution and signing remain open |
