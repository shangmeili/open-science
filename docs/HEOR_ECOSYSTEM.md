...
| Shipped alpha | `heor-methods-watchlist` | Record dated Human checks of official HEOR methods sources, lawful access mode, observed revisions, affected contracts, and required revalidation without fetching or redistributing restricted content | Strict schema `0.1.0` `heor/methods-watchlist.json`, optional hash-bound files under `heor/method-sources/`, portable validator, native audit, and natural-language repair route |
