...
| Shipped | `heor-decision-tree` | Researcher-selected finite, non-recurring event paths within one year; source-bound deterministic replay with an explicit currency, price year, jurisdiction, and perspective | `heor/decision-tree-plan.json` and hash-bound `heor/results/decision-tree.json` |
...
| Shipped alpha | `heor-model-calibration` | Researcher-specified point calibration of bounded unobservable natural-history rates in one homogeneous continuous-time cohort model | Request/result schema `0.1.0`; exact uniformization, fixed grid plus multistart search, immutable evaluation trace, local rank/conditioning diagnostic, mandatory held-out occupancy targets, complete portable replay, native selected-point challenge, and app-owned eight-item Human method review with no automatic model-input replacement |
| Shipped alpha | `heor-semi-markov-microsimulation` | Researcher-specified closed-cohort individual state-transition analysis when time in state or capped event-entry history makes cohort-state expansion unmanageable | Request/result schema `0.1.0`; 2–8 states, 2–4 strategies, 1–3 capped trackers, complete conditional rules and rewards, counter-based SplitMix64 common random numbers, 3–20 replicates, a five-million patient-strategy-cycle cap, sampled traces, complete portable/native replay, and app-owned eight-item Human method review with no automatic strategy selection or downstream replacement |
| Shipped alpha | `heor-rwe-causal-analysis` | Researcher-specified active-comparator new-user target-trial analysis over one local pseudonymous baseline cohort with two strategies, one fixed-horizon binary outcome that may be unobserved, and Human-selected baseline treatment and observation adjustment variables | Request/result schema `0.2.0`; separate unpenalized main-effects Logistic treatment and observation models, untrimmed stabilized source-cohort ATE-IPTW×IPOW, follow-up/overlap/separate-and-combined-weight/observed-row-ESS/three-stage-SMD diagnostics, deterministic arm-stratified bootstrap refits of both models, complete portable replay, native point/diagnostic challenge, and app-owned eight-item Human method review |
