...
partitioned survival, uncertainty analysis, budget
...
| Shipped alpha / release integration pending | `heor-partitioned-survival` | Deterministically calculate three-state PFS/OS occupancy, cost, QALY and incremental results only after endpoint, time-origin, curve-review, typed-fit-output, evaluator, time-grid, crossing, monotonicity, and internal-coherence review. The materialization and PSM Skills, Python engine, native audit/run boundary, analysis-plan approval binding, and natural-language UI are shipped. Hidden treatment-effect composition, competing estimands, unsupported survival families, incoherent occupancy, uncertainty propagation, and release are blocked; release remains unavailable until model-validation and report-package contracts bind the PSM plan/result and a dedicated economic-input contract removes the current unused transition-input requirement. |
