...
structure-neutral economic inputs, partitioned survival, uncertainty analysis, budget
...
| Shipped alpha / release integration pending | `heor-economic-inputs`, `heor-partitioned-survival`, `heor-uncertainty-analysis`, `heor-joint-survival-uncertainty` | Analysis `0.12.0` separates common costs, utilities, cycle settings and discounting from PSM structure; PSM `0.3.0` deterministically calculates three-state PFS/OS occupancy without unused Markov inputs. Uncertainty `0.11.0` remains the explicit fixed-curve economic-only fallback. Uncertainty `0.12.0` consumes one hash-bound joint posterior or paired-patient-bootstrap row across every strategy PFS/OS curve per PSA iteration and rejects independent endpoint sampling, row repair, source-hash drift, crossing, and incomplete grids. Curve-family selection, extrapolation, treatment-effect duration, source-model validity, model-validation/report-package integration, and release remain blocked. |
