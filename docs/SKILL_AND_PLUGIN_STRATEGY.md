...
| Shipped alpha / release integration pending | `heor-economic-inputs`, `heor-partitioned-survival` | Schema `0.12.0` separates common costs, utilities, cycle settings and discounting from PSM structure; schema `0.3.0` deterministically calculates three-state PFS/OS occupancy without unused Markov inputs. The Skills, Python engine, native audits, input provenance, approval binding, and natural-language UI are shipped. Hidden treatment-effect composition, competing estimands, unsupported survival families, uncertainty propagation, model-validation/report-package integration, and release remain blocked. |
