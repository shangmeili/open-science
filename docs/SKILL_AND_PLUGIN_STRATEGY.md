...
| [Paper Search MCP](https://github.com/openags/paper-search-mcp) | MIT candidate at reviewed revision `c8b642183bb725f0a7faec89e58b558df09079d1`; broad cross-source search, full-text, and download functions, with optional Sci-Hub routing. | Quarantine and remove from one-click defaults. Consider only per-source adapters after source rights, egress, request-hash authorization, dependency, adversarial, and cross-platform review. The first-party PubMed/ClinicalTrials.gov route remains narrower and authoritative for product behavior. |
| [BioMCP](https://github.com/genomoncology/biomcp) | MIT candidate at reviewed revision `b586085ac032f079671ede0c08cbf17816183bd3`; current package metadata identifies `biomcp-cli` and `biomcp serve`, while the inherited catalog used stale package/launch values. Its tool surface spans many biomedical sources, credentials, and entities. | Quarantine and remove from one-click defaults. If useful, admit an individual read-only source adapter with source-specific rights, credentials, fixed egress, provenance, Human authorization, and a kill switch; never promote the entire server by label. |
...
`b586085ac032f079671ede0c08cbf17816183bd3`. Both remain quarantined, and the
AI4HEOR default surface contains no third-party one-click MCP process.
