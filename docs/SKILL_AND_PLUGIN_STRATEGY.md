...
| Shipped alpha / release integration pending | `heor-economic-inputs`, `heor-partitioned-survival`, `heor-treatment-effect-duration`, `heor-cost-input-normalization`, `heor-uncertainty-analysis`, `heor-joint-survival-uncertainty` | Analysis `0.13.0` separates common rewards from PSM structure. PSM `0.5.0` binds immutable selected-fit source curves, an explicit two-strategy duration artifact, and exact annual state-cost ingredients. Each cost item reproduces quantity times adjusted unit price and every strategy/state aggregate; method choices remain Human. Uncertainty `0.11.0` / `0.12.0` remains compatible only with legacy analysis `0.12.0` / PSM `0.4.0`; component-level cost uncertainty is not silently approximated. Curve-family selection, extrapolation, source-model validity, model-validation/report-package integration, probabilistic integration across duration alternatives, non-annual cost structures, and release remain blocked. Legacy PSM `0.4.0` remains readable. |
