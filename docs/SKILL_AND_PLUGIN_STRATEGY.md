...
| Shipped alpha / release integration pending | `heor-economic-inputs`, `heor-partitioned-survival`, `heor-treatment-effect-duration`, `heor-uncertainty-analysis`, `heor-joint-survival-uncertainty` | Analysis `0.12.0` separates common rewards from PSM structure. PSM `0.4.0` binds immutable selected-fit source curves plus an explicit two-strategy duration artifact and reports sustained, immediate-stop, and log-linear-waning cost/QALY scenarios. After a declared evidence horizon, the intervention tail is rebuilt from comparator cumulative-hazard increments and one evidence-bound HR; duration is never inferred from the HR point estimate. Uncertainty `0.11.0` remains the fixed-curve economic-only fallback. Uncertainty `0.12.0` binds duration schema `0.1.0`, uses joint-manifest schema `0.2.0`, consumes one complete joint PFS/OS row per PSA iteration, and reports non-base duration alternatives separately rather than pretending to integrate structural uncertainty. Curve-family selection, extrapolation, source-model validity, model-validation/report-package integration, probabilistic integration across duration alternatives, and release remain blocked. Legacy PSM `0.3.0` / joint manifest `0.1.0` remain readable. |
