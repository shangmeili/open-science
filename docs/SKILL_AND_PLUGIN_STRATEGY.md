...
| Shipped alpha / release integration pending | `heor-economic-inputs`, `heor-partitioned-survival`, `heor-uncertainty-analysis` | Analysis `0.12.0` separates common costs, utilities, cycle settings and discounting from PSM structure; PSM `0.3.0` deterministically calculates three-state PFS/OS occupancy without unused Markov inputs. Uncertainty `0.11.0` adds hash-bound economic-input-only DSA/PSA and bounded scenarios, explicitly classifies the output as partial, and requires every fixed PFS/OS curve to be omitted visibly. Hidden treatment-effect composition, competing estimands, unsupported survival families, full survival/joint-curve uncertainty, model-validation/report-package integration, and release remain blocked. |
