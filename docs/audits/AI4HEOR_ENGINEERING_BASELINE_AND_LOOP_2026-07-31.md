...
| P1-QUEUE-001 | P1 | 间歇性复现，待独立处理 | 原生完整流程的较早运行偶尔只发送队列中的第一条，立即复跑可通过；本轮不放宽断言，也不把其他 P1 修复冒充该竞态已关闭 |
...
12. P1-QUEUE-001：以独立失败测试复现原生队列偶发只发送第一条的竞态，再决定最小修复；不得用延长超时或自动重试隐藏。
