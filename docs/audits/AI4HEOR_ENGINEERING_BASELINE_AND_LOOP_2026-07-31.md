...
| P1-SCI-001 | P1 | 经济学基础 loop 已完成定向验证；DSA/PSA、报告和复现包待后续 loop | 当前 0.2 计划必须声明币种、价格年份、地区和分析视角，结果精确回传同一基础；桌面在经济学基础不一致或 `input_sha256` 与当前计划原始字节不一致时均拒绝展示结果。旧 0.1 仅兼容重放并明确标为探索性。已有来源/假设声明、逐节点计算轨迹、增量前沿、CLI 哈希重放、第一方 `$heor-decision-tree`、独立计划/结果路径和启动资源部署；DSA/PSA、报告与复现包未冒充完成 |
