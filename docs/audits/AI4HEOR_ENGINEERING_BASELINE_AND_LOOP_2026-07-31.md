...
| P1-SCI-001 | P1 | 内核、Skill 与自然语言任务执行已完成；专用桌面复核、报告和复现包待后续 loop | 已有独立 schema、黄金案例、来源/假设声明、逐节点计算轨迹、增量前沿、CLI 哈希重放、第一方 `$heor-decision-tree`、独立计划/结果路径、多语言目录、完整 53-Skill 启动准入和真实 Tauri 资源部署；结果通过 `input_sha256` 精确绑定。DSA/PSA、专用桌面复核、报告与复现包未由本轮冒充完成 |
...
9. P1-SCI-001：首版确定性内核、第一方 Skill 与自然语言任务执行已完成；后续分别为专用桌面审查、报告与复现包建立独立合同。P1-SCI-002 仍需先建立人工可核算基准，禁止与界面修复混做。
