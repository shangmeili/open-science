...
- 本轮开始前的已提交基线：`3ff7136`；更早已完成业务修复均保留为独立提交，本轮不重复修改。
...
| AI 调用与任务执行 | OpenCode 本地 sidecar，HTTP/SSE，模型提供商可配置 | 完成调用的提供商、模型、时间、token、缓存 token、结束原因和运行时报告费用已归一化；新的 HEOR 调用记录固定前导提示的精确 SHA-256 与回复语言；对话正文不进入调用账本 |
...
| P1-AI-001b2 | P1 | 待单独处理 | 既有工作区的 Harness 可由用户修改且不会被新版资源静默覆盖；调用账本尚未记录当次实际工作区 Harness 的精确指纹 |
...
8. P1-AI-001a 和 P1-AI-001b1：不记录密钥、研究者文本或回答正文的模型调用账本，以及应用固定 HEOR 前导提示指纹已完成；P1-AI-001b2/b3 继续分别补齐当次实际 Harness 指纹和研究产物关联，P1-AI-001c 单独建立研究者可见审计界面。
