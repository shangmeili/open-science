...
| P1-AI-001b2 | P1 | 实现与合同已修复；新包执行待打包轮 | 研究者已同意方案 A 和本地派生数据边界；固定 OpenCode 1.17.13 源码的最小补丁在供应商请求前把插件处理后的最终系统块指纹写入准确助手消息，标题等无对应消息的辅助调用不写；源码、补丁、Bun、许可证和发布夹具均受门禁约束 |
...
9. P1-AI-001a、P1-AI-001b1 和 P1-AI-001b2：内容无关模型调用账本、固定 HEOR 前导提示指纹，以及当次实际最终系统块指纹的实现与合同已完成；P1-AI-001b2 仍需由当前源码新 DMG 执行打包夹具，P1-AI-001b3 继续补齐研究产物关联，P1-AI-001c 单独建立研究者可见审计界面。
...
| 系统上下文安装包执行门禁 | `verify_packaged_opencode_fixture.py` + macOS 构建工作流 | 单元合同 2/2 与 CI 接线合同通过；必须对当前源码新 DMG 执行后才能形成安装包证据，旧 DMG 不计入 |
...
- 验收结果：固定补丁合同 4/4、上游 19/19、SDK/桥接 21/21、Rust 账本 4/4、前端 768/768、Rust 376 通过/1 项既有忽略、HEOR 188/188、本轮相关开发合同 234 项与发布合同 33 项通过；类型、ESLint、Rust 格式、生产构建和 41 来源/445 文件资源预检通过。固定 Bun 1.3.14 已从当前补丁重新构建 x64 Mach-O sidecar，版本冒烟为 `1.17.13-ai4heor.1`。
- 回滚方式：回退本 loop 独立提交；不改写或删除已形成的本地账本和研究产物。回滚后新字段会被旧客户端忽略，旧消息与旧账本仍可读取。
- 剩余风险：当前源码尚未生成新 DMG，因此打包后的真实 sidecar/provider 请求/持久助手消息三方一致性仍由下一打包轮执行；Windows、Apple Silicon 和 Linux 二进制需各自原生 CI 重建。指纹证明当次系统块字节一致，不证明指令本身科学正确，也不建立模型调用到研究产物的因果来源；后两项分别保留在 P1-AI-001b3 与 P1-AI-001c。
