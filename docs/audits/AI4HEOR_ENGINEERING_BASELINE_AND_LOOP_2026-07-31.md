...
- P1-AI-001b2 开始前的已提交基线：`38babd9`；P1-AI-001b3 实施前的已提交基线：`bc71b1b`；P1-TEST-001b1 实施前的已提交基线：`38a5687`；P1-TEST-001b2 实施前的已提交基线：`076180f`；P1-TEST-001b3 实施前的已提交基线：`14254f8`；P1-TEST-001b4 实施前的已提交基线：`1cf99a8`；P1-TEST-001b5 实施前的已提交基线：`6f48f57`；P1-TEST-001b6 实施前的已提交基线：`93912be`；P1-TEST-001b7 实施前的已提交基线：`7e26b0d`；P1-TEST-001b8 调查前的已提交基线：`27802d4`；P1-TEST-001b9 安装包验证基线：`b480c26`；P1-TEST-001b10 供应商失败恢复实施前基线：`d275c0a`；P1-TEST-001b11 导入—执行—导出验收实施前基线：`ab3b2d5`；P1-TEST-001b12 外部安装包 UI 验收实施前基线：`20c415a`；P1-TEST-001b13 安装包任务回复链实施前基线：`0fdd6dc`；P1-TEST-001b14 封装 OpenCode 证据闭环实施前基线：`d34aebc`；P1-TEST-001b15 DMG 跨阶段字节绑定实施前基线：`19ad8b8`；P1-TEST-001b16 核心 macOS 包检查证明实施前基线：`ff47b0b`；P1-TEST-001b17 隔离启动清理证明实施前基线：`3934cb8`；P1-AI-001c 研究者可见审计实施前基线：`85fa920`；P1-AI-001d 精确对话定位实施前基线：`5040b1f`；P1-QUEUE-001 队列竞态实施前基线：`5c0a6ac`；P1-SCI-001a 决策树 Skill 与任务执行实施前基线：`68a9076`；P1-SCI-001b 专用桌面复核实施前基线：`499f968`；P1-SCI-001c 决策树经济学基础实施前基线：`c098014`；P1-SEC-002b OOM 通告修复实施前基线：`04b2142`；P1-SEC-002c UUID 可达性复核实施前基线：`a2201b3`；P1-SEC-002d 内部路由边界实施前基线：`2782100`。更早已完成业务修复均保留为独立提交，本轮不重复修改。
- 基线原则：在现有 Open Science/Tauri 技术栈上增量推进；AI 负责辅助推理，正式研究计算由可验证的确定性模块完成；科学、隐私、兼容性和公开接口决策保留 Human-in-the-loop。
- 已完成 loop 范围：不可信 HTML 预览边界、同主版本 JavaScript CPU 拒绝服务补丁、安装后 OpenCode 的鉴权 HTTP 就绪证据、安装后前端 bootstrap 证据合同、经研究者确认的两策略 PSA 零 INMB 并列口径、PPTX 预览中 ECharts `lines` 系列通告的不可达性门禁、短期确定性决策树内核、经济学基础、DSA/PSA、来源绑定的受限亚组分析、报告、复现包、专属独立验证和确定性发布重放、第一方 Skill、自然语言任务执行路径和按精确输入字节绑定的专用桌面复核、不含对话正文的模型调用用量/费用元数据账本、应用固定 HEOR 前导提示的精确指纹和回复语言记录、模型调用与工具产物/运行的准确关联、研究者可见调用详情、从审计记录返回精确助手/工具动作的对话定位，以及 OpenCode 双重空闲事件导致的排队竞态。
...
| HEOR 确定性计算 | Python 版本化计算模块 + Rust 授权、审计和哈希绑定 | 决策树当前 schema 0.2.0 强制声明币种、价格年份、地区和分析视角；旧 0.1.0 仅兼容确定性重放并明确标为探索性；CLI 重放、第一方 `$heor-decision-tree`、原子结果写入和启动资源门禁已连接；既有 Markov/PSM 公式、参数和随机数列不变 |
...
| Human-in-the-loop | 关键科学定义、证据采用、结构与发布决策由研究者确认 | PSA 并列口径在研究者同意后实施；决策树只设置有实际责任对象的“独立验证→发布”两道关卡，不伪造 Markov/BIA 审批 |
| 文件预览 | React inspector + Tauri loopback preview server | HTML 改为被动展示；源码查看和外部打开保留 |

当前仓库已有主要单元/组件、确定性 HEOR、Rust 原生与资源门禁测试。macOS 与 Windows 发布脚本已有真实主机级安装、进程、工作区和清理检查，并已补上 OpenCode 鉴权 HTTP 与前端 bootstrap 合同。测试专用 macOS WebDriver 变体已在真实 Tauri/WKWebView 中完成基础导航、被动 HTML 预览、项目/独立任务作用域、队列、Human 问题、权限、provider 失败恢复，以及真实项目导入—项目内任务执行—确定性 DOCX/PDF/XLSX 报告导出验证。macOS DMG 验证器现另用发布二进制外部的 Accessibility 语义驱动按精确 PID 验证窗口、任务入口、输入框以及通过无密钥本地模型夹具完成任务创建—发送—回复显示，不把 WebDriver 编进正式应用；当前源码尚未重新生成 DMG，因此当前提交的安装包字节级证据仍待下一次打包生成。
...
| P1-TEST-001b | P1 | 调试构建主要交互分支及导入—执行—导出已完成；安装包内 A1 权限已验证；安装版导航、输入和本地回复链驱动已实现，当前源码 DMG 待生成证据 | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界、任务作用域隔离、队列排序/删除/逐条发送、OpenCode `question` 回答恢复、provider 400 失败恢复、危险 bash 命令的一次授权/拒绝，以及项目原生导入、受管副本内项目任务执行、确定性 DOCX/PDF/XLSX 报告导出与当前性审计；外部源目录保持不变。DMG 验证器现从外部按精确 PID 驱动正式应用的语义辅助功能树；现有安装版隔离副本已通过窗口、导航、输入及本地夹具任务创建—发送—回复显示，当前源码 DMG 仍待字节级实证 |
| P1-TEST-001b14 | P1 | 已修复；当前源码 DMG 待生成证明 | macOS 工作流原先运行封装 OpenCode 的系统上下文与权限持久化夹具，却没有把结果写入验证 JSON；发布证据可只靠检查名称声称通过。现将结果压缩为无正文、无哈希、无内部标识的有界证明并原子追加，声明任一检查时必须同时声明配对检查并通过严格字段校验 |
| P1-TEST-001b15 | P1 | 已修复；下一提交 DMG 待重建 | macOS 验证 JSON 与发布证据分别记录 DMG 哈希，但原先没有交叉核对；两阶段之间替换 DMG 仍可能产生形式合法但不指向同一制品的记录。夹具运行前和证据校验时现均强制文件名、目标和 SHA-256 一致 |
| P1-TEST-001b16 | P1 | 已修复；当前源码 DMG 待重建 | 发布证据原先可声明包元数据、Mach-O 架构、科研资源、包内 HEOR 测试和 sidecar 五项核心检查，却不要求验证 JSON 提供精确配对证明。现要求五项成组出现，并逐项绑定 Info.plist、目标架构、资源数量、包内测试成功和实际 sidecar 版本 |
| P1-TEST-001b17 | P1 | 已修复；当前源码 DMG 待重建 | macOS 包验证器实际清理首次启动、工作区隔离和任务回复三个临时应用副本，但发布证据只强制消费了工作区隔离清理结果，且任务回复清理结果被压缩丢失。现三条路径均须提供清理成功证明 |
...
| P1-SCI-001 | P1 | 已修复 | 决策树当前 0.2 计划、证据综合、不确定性计划和两项确定性结果由独立验证 schema 0.3 精确绑定；旧验证 schema 不得进入正式关卡。具名独立审阅者确认后，具名发布责任人才能执行两项确定性重放并发布；数值漂移失败关闭，不要求或伪造 Markov 概念模型、Markov 分析计划或 BIA 审批 |
| P1-SCI-002 | P1 | 已修复 | 当前决策树支持受限、可复算的描述性亚组分析：预设/事后状态、人群定义与占比、各亚组完整模型、证据提取及当前输入字节均精确绑定；结果保持等待研究者复核，不声称交互作用、治疗效果修饰或亚组决策规则 |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML、项目/独立任务作用域、消息队列、Human 问题恢复、provider 失败恢复、危险命令一次授权/拒绝，以及项目导入—项目内任务—确定性报告导出 E2E 已完成；当前安装包也已验证“始终允许”的精确作用域、重启复用、撤销和重新询问。外部 Accessibility 驱动现可在不修改发布二进制的前提下验证 DMG 应用的窗口、任务入口、输入框和本地夹具回复链；下一次打包必须以当前提交 DMG 生成同样证据。
...
9. P1-SCI-001：决策树 0.2 经济学基础、DSA/PSA、报告、复现包、专属独立验证与确定性发布重放均已完成；P1-SCI-002 的受限描述性亚组分析也已用人工核算案例独立完成。决策树与 Markov/PSM 使用各自准确的工件图和关卡，不互相冒充（已完成）。
...
15. P1-TEST-001b14：封装 OpenCode 的系统上下文一致性与权限持久化结果必须进入同一份 macOS 验证 JSON；只有进程退出成功而没有有界证明时不得记录为通过（已完成代码与合同，当前源码 DMG 待执行）。
16. P1-TEST-001b15：DMG 从内容验证、OpenCode 夹具到最终发布证据必须保持同一文件名、目标和 SHA-256；任一步之间替换字节均 fail-closed（已完成代码与合同）。
17. P1-TEST-001b16：五项 macOS 核心包检查必须成组声明，并由同一验证 JSON 提供精确、相互一致的 Info.plist、架构、资源、包内 HEOR 测试和 sidecar 版本证明（已完成代码与合同，当前源码 DMG 待执行）。
18. P1-TEST-001b17：首次启动、工作区隔离和安装版任务回复三次隔离应用运行都必须清理包内应用/OpenCode 进程，并把成功结果写入和绑定到发布证据（已完成代码与合同，当前源码 DMG 待执行）。
...
| 安装包 UI 修复前复现 | 三项发布合同测试 | 3/3 按预期失败：缺少外部 Accessibility 驱动、CI 检查和有界发布证据校验 |
| 安装包 UI 定向合同 | macOS 分发、工作流与发布证据三项测试 | 3/3 通过；探针只按 AX 角色和当前中英文标签定位，不使用鼠标坐标或把 WebDriver 编进应用 |
| 现有安装版隔离 UI 冒烟 | `verify_first_launch(/Applications/AI4HEOR.app, x86_64)` | 通过；临时复制、隔离 HOME 和精确进程路径下验证窗口、新建任务、输入框编辑/清空、任务文件、插件与技能、工作区隔离及进程清理；该证据不冒充当前提交 DMG |
| 安装包 UI 发布层回归 | `python3 -B -m unittest discover -s scripts/release -p 'test_*.py' -q` | 55/55 通过；类型检查、ESLint 和生产构建同时通过 |
| 安装包回复链修复前复现 | 三项发布合同测试 | 3/3 按预期失败：语义驱动没有发送/回复分支、工作流未声明检查、发布证据可在没有回复证明时通过 |
| 现有安装版隔离回复链 | `verify_first_launch(/Applications/AI4HEOR.app, native)` | 通过；本地夹具收到请求，正式应用创建任务、提交固定测试消息并显示固定回复，三个隔离应用进程均清理；该结果只证明机制，不冒充当前提交 DMG |
| 安装包回复链发布层回归 | 发布合同、产品文档、Python/Swift 语法与差异检查 | 发布合同 56/56、产品文档 15/15 通过；`py_compile`、Swift typecheck 与 `git diff --check` 通过 |
| 封装 OpenCode 证据修复前复现 | 夹具、发布证据与工作流三项合同 | 3/3 按预期失败：夹具没有有界写入函数、证据验证器接受空声明、工作流未传入验证文件 |
| 封装 OpenCode 证据定向回归 | `test_packaged_opencode_fixture.py`、发布证据与 macOS 工作流合同 | 夹具 10/10；系统上下文与权限检查必须成对出现，证明仅保留回复完成、流式、指纹匹配和四项权限生命周期布尔值 |
| DMG 跨阶段绑定修复前复现 | 发布证据与封装 OpenCode 两项合同 | 2/2 按预期失败：验证 JSON 哈希被替换仍可通过；夹具没有目标 DMG 绑定函数 |
| DMG 跨阶段绑定回归 | 完整发布合同及当前 1.0.0 内部测试证据 | 发布合同 59/59；新门禁对提交 `19ad8b8` 生成的 DMG、验证 JSON 和发布证据三方一致性校验通过 |
| 核心包证明修复前复现 | 发布证据与 macOS 验证输出两项定向合同 | 2/2 按预期失败：声明五项核心检查时可缺少精确证明；包内 HEOR 测试成功没有进入验证 JSON |
| 核心包证明回归 | 定向合同、完整发布合同、产品文档与语法/差异检查 | 定向 2/2、发布合同 61/61、产品文档 15/15 通过；Python 语法和 `git diff --check` 通过。既有标签发布信任测试补齐合法核心包前置证明后继续验证签名/公证门禁，未放宽断言 |
| 隔离启动清理证明修复前复现 | 首次启动证据与任务回复验证输出两项合同 | 2/2 按预期失败：首次启动清理标志为假仍被接受；任务回复隔离启动的清理结果没有写入最终验证 JSON |
| 隔离启动清理证明回归 | 定向合同、完整发布合同、产品文档与语法/差异检查 | 定向 2/2、发布合同 62/62、产品文档 15/15 通过；Python 语法和 `git diff --check` 通过 |
...
| 决策树经济学基础修复前复现 | 新增 0.2 经济学基础、旧版重放和桌面一致性测试 | Python 0.2 因 `economic_basis` 为不支持字段而失败，旧 0.1 没有探索性警告；桌面把有效 0.2 计划判为无效；Skill 模板仍为 0.1 |
| 决策树经济学基础定向回归 | 决策树核心、Skill 黄金案例、桌面只读复核 | Python 12/12、核心 Skill 11/11、桌面决策树 4/4 通过；0.2 缺失/非法基础、计划与结果基础或引擎版本不一致均失败关闭，0.1 保持 0.1 引擎重放且不新增基础字段 |
| 决策树经济学基础全量回归 | 前端、HEOR、Rust、开发/发布合同、类型、ESLint、生产构建、diff、产品文档和资源预检 | 前端 118 文件/797 项；HEOR 190/190；Rust 378 通过/1 项既有公网测试忽略及资源暂存 2/2；开发合同 400/400；发布合同 62/62；产品文档 15/15；41 来源/450 文件；其余全部通过 |
| 决策树独立验证与发布重放修复前复现 | Rust 审批顺序、前端复核与第一方 Skill 合同 | 现有门禁要求虚假的三项 Markov 前置批准并使用 Markov/PSM/BIA 重放路径；专属验证模板不存在；前端类型与 lint 门禁准确拦截未收紧路径 |
| 决策树独立验证与发布重放定向回归 | Rust 验证/审批/报告、便携验证器、前端专用复核、Skill Creator | 12 项决策树 Rust 测试、9 项便携验证合同、7 项前端复核与 Skill 官方结构校验通过；旧验证 schema、结果漂移、撤销后的下游发布和重放值变化均失败关闭 |
| 决策树独立验证与发布重放全量回归 | 前端、HEOR、Rust、开发/发布合同、类型、ESLint、格式、构建、diff、资源预检与原生 E2E | 前端 311 文件/802 项；HEOR 197/197；Rust 390 通过/1 项既有公网测试忽略及资源暂存 2/2；开发合同 414/414；发布合同 62/62；43 来源/460 文件；真实 Tauri/WKWebView 的既有任务、队列、Human、权限、恢复、文件、导入和确定性导出均通过 |
...
- 当前行为：`pnpm audit --prod --json` 报告 `uuid 8.3.2 / 10.0.0` 命中 [GHSA-w5hq-g745-h8pq](https://github.com/advisories/GHSA-w5hq-g745-h8pq)。通告只在调用者把过小的输出缓冲区传给 `v3`、`v5` 或 `v6` 时成立；生产依赖路径分别为 `exceljs -> uuid 8.3.2` 和 `pptx-preview -> uuid 10.0.0`。
- 目标行为：不能仅凭版本告警宣称应用可被利用，也不能无证据忽略。必须精确约束两条实际消费者的 API：只要任一依赖开始导入受影响 API、增加调用点或改变 UUID 参数形态，发布门禁立即失败并要求重新评估。
- 首次失败证据：新增门禁首次运行准确失败，因为最初只读检索只发现 ExcelJS 的主源码调用，实际安装包还含相同逻辑的 ES5 和两份分发副本。没有把期望值改成“至少一处安全”，而是逐一核对四份 ExcelJS 调用点和 PPTX 预览入口。
- 根因结论：四份 ExcelJS 消费者都只解构 `v4: uuidv4`，且两次调用均无参数；PPTX 预览也只 `import { v4 } from "uuid"`。受影响的 `v3/v5/v6 + caller buffer` 组合当前不可达。审计器按安装版本报告通告，不分析调用图，因此告警保留并不等于存在可利用的产品路径。

### 最小控制、验收与边界

- 最小控制：只扩展现有 `scripts/dev/test_js_security_policy.py`，固定审核过的 ExcelJS 源码/ES5/分发调用点集合与 PPTX 预览 UUID 导入；没有升级或覆盖 UUID，没有修改 ExcelJS/PPTX、业务代码、研究数据、模型调用、HEOR 公式、schema 或界面。
...
- 当前行为：测试特性 Tauri/WKWebView 已覆盖真实任务交互，但 WebDriver 插件明确不进入普通或发布构建。DMG 验证器只验证精确安装副本的进程、OpenCode 鉴权 HTTP、前端 bootstrap、工作区与独立 sidecar 夹具；这些证据不能证明安装包窗口中的“新建任务”可点击、输入框可编辑或核心入口可见。
- 可行性调查：本机没有完整 Xcode，不能建立或本地验证 XCUITest；正式安装应用的 WKWebView 仍通过 macOS Accessibility 暴露语义树。只读探针从当前安装应用遍历 3135 个节点并识别 380 个可操作元素，可按 `AXButton`、`AXTextArea` 与当前中英文标签定位目标，不需要屏幕坐标或应用内测试服务器。
- 目标行为：DMG 验证器复制出的精确 `.app` 在隔离 HOME 下启动后，由发布二进制外部的系统辅助功能驱动按精确 PID 验证 AI4HEOR 窗口、新建任务导航、任务输入框编辑并清空、任务文件入口和插件与技能入口；证据只保存五个布尔值。
- 失败测试：先加入 macOS 驱动合同、工作流检查和发布证据约束，修复前 3/3 稳定失败，分别对应脚本缺失、`installed-task-ui` 未接入 CI 以及证据验证器忽略该检查。
- 根因：此前把“发布二进制不应含 WebDriver”与“安装包 UI 无法自动验证”错误地绑定在一起，没有利用系统提供的进程外语义辅助功能边界。

### 最小修复、验收与边界

- 最小修复：新增一份第一方 Swift Accessibility 探针；`verify_macos_package.py` 只在 `--verify-first-launch` 的第一个隔离启动中调用它，并在进程终止前收集有界结果；发布证据只在 macOS 接受精确五字段全真对象，而且必须同时具备首次启动、前端启动、OpenCode 鉴权和工作区创建四项配套检查；构建工作流加入对应检查。应用源码、Tauri feature、发布二进制、模型配置、研究数据、科研计算、schema、权限和界面均未修改。
- 语义与隐私边界：探针只按 `AX` 角色和当前 `zh-Hans` / `en` 资源标签定位，测试同时锁定这些标签；不使用 `CGEvent`、鼠标坐标、截图或 OCR。输入框只暂存固定哨兵并立即清空，运行于临时 HOME，进程结束后目录删除；证据不保存 PID、路径、界面文本、研究内容或屏幕内容。
- 验收结果：修复后三项定向合同 3/3；现有 `/Applications/AI4HEOR.app` 的临时隔离副本真实通过五项 UI 证明、工作区隔离与进程清理；发布层 55/55，类型检查、ESLint、生产构建、Python 语法和差异检查通过。
- 剩余风险：当前源码尚未重建 DMG，因此现有安装版的真实冒烟只证明驱动机制，不是当前提交安装包证据；正式证据必须由下一次当前源码 DMG 验证生成。GitHub 官方 runner 镜像文档确认 macOS 15 Intel 环境和 Swift/Xcode 工具，但未承诺 Accessibility TCC 授权，首次 CI 原生执行仍待验证并将 fail-closed。该 loop 只覆盖可见导航与输入，不覆盖安装包 UI 发起本地 provider 请求并显示回复；后者保留为独立 P1-TEST-001b13。
- 回滚方式：回退本 loop 独立提交即可；不会改写用户数据、研究产物、依赖锁文件或安装应用。

## Loop P1-TEST-001b13

### 发现、复现与根因

- 当前行为：发布流程分别证明了封装 OpenCode 可调用无密钥本地模型夹具，以及正式应用窗口可导航、输入框可编辑；两条证据没有贯通，无法证明 DMG 内真实界面能够从“新建任务”完成输入、发送、会话创建和回复显示。
- 目标行为：用第三个隔离 HOME 预置仅供验收的本地 provider 与非秘密凭据；正式应用必须通过自身捆绑的 OpenCode 发出固定测试请求，本地夹具必须收到请求，窗口必须显示固定回复。发布证据只保存任务已创建、消息已提交、provider 已收到、回复已显示四个布尔值。
- 失败测试：先扩展语义驱动合同、macOS 工作流合同和发布证据合同；修复前 3/3 稳定失败，分别证明驱动没有发送/回复分支、CI 未声明 `installed-task-reply`，以及缺少回复证明仍可被记录为通过。
- 根因：原有 OpenCode 夹具直接调用 sidecar HTTP，原有安装 UI 冒烟只驱动任务入口；缺少一项在隔离安装应用中把这两条路径串联起来的验收。

### 最小修复、验收与边界

- 最小修复：复用已有 Anthropic 兼容本地夹具，不新增模型协议；在隔离应用私有 runtime 中写入测试 provider 配置和非秘密凭据；扩展同一份外部 Swift Accessibility 驱动，按当前中英文语义标签输入固定消息、按下发送并等待固定回复；发布证据要求四字段全真，并强制同时具备首次启动配套检查与 `installed-task-ui`。应用业务代码、Tauri feature、正式二进制、用户模型设置、研究数据、科学计算和 schema 均未修改。
- 数据与隐私边界：测试 HOME、工作区、配置和凭据均位于临时目录；不读取现有用户配置，不访问外部模型或互联网，不保存提示词、回复、端口、PID、路径或凭据。退出时精确终止临时应用及 sidecar，临时目录由上下文清理。
- 验收结果：三项定向失败合同修复后 3/3；现有 `/Applications/AI4HEOR.app` 的临时隔离副本真实完成任务创建—发送—本地 provider 请求—回复显示，界面、工作区隔离和进程清理均通过；发布合同 56/56、产品文档 15/15、Python 语法、Swift typecheck 与差异检查通过。
- 剩余风险：当前源码尚未重建 DMG，因此现有安装版结果只证明验收机制和既有安装字节的真实路径，不是当前提交产物证据。下一次当前源码 DMG 必须由相同验证器生成 `installed_task_reply` 四项证明；CI 主机若没有 Accessibility 授权将 fail-closed，不能降级为直接 sidecar HTTP 证明。
- 回滚方式：回退本 loop 独立提交即可；不会迁移或改写用户数据、研究产物、依赖锁文件或已安装应用。

## Loop P1-TEST-001b14

### 发现、复现与根因

- 当前行为：`verify_packaged_opencode_fixture.py` 确实会让 DMG 内 OpenCode 完成本地 provider 回复、核对当次实际 system 块指纹，并验证项目级精确权限的保存、重启复用、撤销和重新询问；但脚本只向标准输出打印摘要。随后 `release_evidence.py` 接受 `opencode-system-context-audit` 与 `opencode-permission-persistence` 两个检查名称，却不要求验证 JSON 中存在对应证明。
- 目标行为：夹具必须把结果以有界、无研究内容的结构原子追加到同一份 macOS 验证 JSON；发布证据只有在两项检查成对声明且证明结构精确、所有布尔值为真时才接受。
- 失败测试：新增夹具有界写入、发布证据拒绝空声明和工作流参数贯通三项合同；修复前分别因函数不存在、空声明被接受、夹具调用未传验证文件而失败。
- 根因：夹具最初作为命令级 fail-closed 门禁接入，后续引入可下载发布证据时只记录了检查名称，没有把命令执行结果转换成可独立复核的数据合同。

### 最小修复、验收与边界

- 最小修复：从既有夹具结果派生精确证明，只保留助手回复完成、provider 流式响应、`ai4heor.system-context/v1` 指纹已与当次请求匹配，以及项目级规则、重启复用、撤销、撤销后重新询问四项权限布尔值；不保留 system 哈希、块数、请求计数、提示词、回复、凭据、路径、端口或内部 ID。夹具拒绝覆盖已有证明，并通过同目录临时文件加 `os.replace` 原子写入。工作流把 `verification` 文件显式传给夹具；发布证据要求两项检查成对且证明字段无多无少。
- 验收结果：三项定向合同通过；夹具合同 10/10；完整发布合同 57/57、产品文档 15/15、Python 语法和差异检查通过。产品业务代码、模型协议、权限实现、用户数据、科研计算与 schema 未修改。
- 剩余风险：本轮证明合同尚未由当前源码新 DMG 生成；旧 DMG 的直接夹具成功仍是真实命令执行证据，但旧发布证据 JSON 没有新字段，不能追溯补写或升级宣称。下一次打包必须现场生成证明，不能从日志或历史摘要回填。
- 回滚方式：回退本 loop 独立提交即可；不会改写用户配置、研究数据、历史权限或安装应用。

## Loop P1-TEST-001b15

### 发现、复现与根因

- 当前行为：`verify_macos_package.py` 在验证 JSON 的 `bundle` 中记录 DMG 文件名、目标和 SHA-256，`release_evidence.py record` 又从传入制品独立生成哈希记录；验证器只检查各自格式和最终文件字节，没有要求两份记录相等。封装 OpenCode 夹具追加证明前也没有确认它打开的 DMG 与前一步验证 JSON 是同一字节。
- 目标行为：夹具启动前必须重算目标 DMG SHA-256 并与验证 JSON 比对；最终发布证据必须强制验证 JSON 的文件名、目标、SHA-256 与唯一 `dmg` 制品记录三方一致。
- 失败测试：一项测试把验证 JSON 的哈希替换为另一合法 64 位哈希，旧发布验证仍通过；另一项测试要求夹具在 DMG 字节被替换后拒绝，旧实现因无绑定函数而失败。
- 根因：前两轮分别补齐了“证明内容”和“证明结构”，尚未建立从内容验证、夹具执行到最终制品记录的跨阶段引用完整性。

### 最小修复、验收与边界

- 最小修复：`release_evidence.py` 在声明 `dmg-app-copy` 时要求 macOS 平台、唯一 DMG 制品及精确三字段 `bundle`，并交叉比对文件名、目标和哈希；封装 OpenCode 夹具读取验证 JSON，拒绝符号链接、缺失/多余绑定字段、非 macOS 目标或哈希不一致，并在昂贵夹具运行前完成检查。应用、模型、权限、研究数据、科学计算、schema 和打包字节均未修改。
- 验收结果：两项定向失败合同修复后 2/2；完整发布合同 59/59、产品文档 15/15、Python 语法与差异检查通过；本轮门禁对提交 `19ad8b8` 已生成的 DMG、验证 JSON 和发布证据再次校验通过。
- 剩余风险：`19ad8b8` DMG 是本轮代码修改前生成的内部测试产物；本轮脚本门禁提交后必须重新构建并现场生成新证据，不能把旧 DMG 提升为新提交产物。Developer ID、公证和 Gatekeeper 仍需发布凭据及 CI runner。
- 回滚方式：回退本 loop 独立提交即可；无用户数据、研究结果或安装应用迁移。

## Loop P1-TEST-001b16

### 发现、复现与根因

- 当前行为：macOS 发布证据可以声明 `bundle-metadata`、`macho-architecture`、`scientific-resources`、`packaged-heor-tests` 和 `bundled-sidecars`，但验证器不要求这些名称与同一份包验证 JSON 中的实际结果配对。因而检查名称存在并不能独立证明 Info.plist、架构、科研资源、包内 HEOR 测试和 sidecar 版本均来自该包且通过。
- 目标行为：五项核心包检查必须成组声明；发布证据逐项核对应用标识与版本、目标架构、资源文件数、包内 HEOR 测试成功以及 OpenCode/uv 的实际版本输出。任一字段缺失、额外、错误或任一检查单独出现均 fail-closed。
- 失败测试：先新增两个定向合同；旧实现一项因没有核心检查证明常量和校验而失败，另一项因包内 HEOR 测试完成后未记录成功布尔值而失败。两项均在实现前稳定复现。
- 根因：包验证脚本已执行这些检查并保存部分摘要，但发布证据层只把检查名称当作已完成事实；`packaged-heor-tests` 甚至没有对应的有界输出字段。这与前两轮修复的跨阶段字节绑定属于不同的证明完整性缺口。

### 最小修复、验收与边界

- 最小修复：在包内 188 项 HEOR 测试成功返回后才写入 `packaged_heor_tests=true`；发布证据把五项检查定义为不可拆分集合，并严格比对 Info.plist 五字段、目标到 Mach-O 架构映射、资源文件数、sidecar 实际版本与匹配架构执行标记。没有修改应用、科研公式、模型、提示词、权限、用户数据、研究 schema 或打包内容。
- 验收结果：两项修复前失败合同转绿；完整发布合同 61/61、产品文档 15/15、Python 语法和差异检查通过。完整套件最初准确发现标签发布信任测试的合法夹具缺少新核心证明；只补齐该前置证明后，测试继续要求 Developer ID、硬化运行时、安全时间戳、公证票据和 Gatekeeper 信任，没有删除、跳过或放宽断言。
- 剩余风险：提交 `19ad8b8` 生成的现有 DMG 在业务与包内测试上通过，但其验证 JSON 没有本轮新增的 `packaged_heor_tests` 字段，且早于本轮脚本提交，不能晋升为当前源码制品。必须从包含本 loop 的干净提交重建 DMG 并现场生成新证据；未签名、未公证的包仍只能作内部测试。
- 回滚方式：回退本 loop 独立提交即可；不迁移或改写用户数据、研究产物、安装应用或既有证据文件。

## Loop P1-TEST-001b17

### 发现、复现与根因

- 当前行为：macOS 验证器会依次启动首次使用、Open Science 共存隔离和本地模型回复三个临时应用副本，`launch_isolated_app` 在每次返回前均按精确可执行文件路径终止应用与封装 OpenCode，残留进程会抛错。但发布证据只检查了 `workspace_isolation.cleanup_verified`；首次启动顶层的同名字段即使为假仍被接受，任务回复启动已有的清理结果则未进入最终有界证明。
- 目标行为：三次隔离运行均必须完成清理并形成可消费证明；任一清理字段缺失或为假都不能生成有效发布证据。
- 失败测试：先把首次启动证明的 `cleanup_verified` 改为假，旧校验仍通过；再要求任务回复结果保存该次启动的清理值，旧验证输出没有对应字段。两项均在实现前稳定失败。
- 根因：进程清理在执行层已经 fail-closed，但证据压缩与消费规则是在不同轮次逐步加入，任务回复只复制了可视交互和 provider 请求布尔值，遗漏运行生命周期事实。

### 最小修复、验收与边界

- 最小修复：首次启动配套检查额外要求顶层 `cleanup_verified=true`；任务回复启动从自身 `reply_launch` 复制清理成功布尔值，并把它纳入精确字段集合。现有工作区隔离清理门禁保持不变。没有修改应用业务代码、进程终止实现、提示词、模型、科学计算、权限、用户数据或研究 schema。
- 验收结果：两项失败测试转绿；完整发布合同 62/62、产品文档 15/15、Python 语法和差异检查通过。没有以延时、忽略残留进程或放宽既有断言换取通过。
- 剩余风险：当前目标目录中的 DMG 来自 `19ad8b8`，其验证 JSON 缺少任务回复清理字段，且早于本 loop，不能按当前证据合同晋升。包含本 loop 的干净提交仍需重建并执行三次真实隔离启动；未签名、未公证时仍仅限内部测试。
- 回滚方式：回退本 loop 独立提交即可；不会删除用户进程、迁移数据、改写研究结果或追溯修改既有证据。
...
## Loop P1-SCI-001c：决策树经济学基础与旧版重放边界

- 当前行为与目标：决策树 schema/engine `0.1.0` 能进行来源绑定和确定性计算，却不记录币种、价格年份、地区或分析视角；其金额无法支撑正式经济学解释。目标是在不迁移历史文件、不改变概率树公式或既有黄金结果的前提下，以当前 `0.2.0` 强制声明一套完整经济学基础，并把 `0.1.0` 限定为探索性重放。
- 修改前失败测试与根因：新增核心测试首先稳定失败于 `analysis contains unsupported field(s): economic_basis`；旧版结果没有包含 `exploratory` 的警告；桌面将有效 0.2 计划判为无效；Skill 模板仍固定为 0.1。根因是首版计算合同只解决了结构、来源和数值重放，没有建立金额的适用语境，也没有为当前与历史契约分流。
- 最小修复：新增不可变经济学基础对象，当前 0.2 只接受精确四字段：三位大写 ASCII 币种、1900–2100 的整数价格年份、无首尾空白/控制字符的地区和分析视角。结果原样回传同一对象，engine 为 0.2；旧 0.1 仍使用 engine 0.1、不接受或输出经济学基础，并增加探索性警告。Skill 模板和合同升级为 0.2，但验证器按实际计划报告 schema，因此既有 0.1 文件仍可重放。桌面只读解析器要求计划、结果、engine 和四字段完全一致，否则隐藏结果；界面仅展示基础，不做汇率、通胀或视角推断。
- 科学与兼容边界：本 loop 没有变更树拓扑、分支概率、成本/QALY公式、增量分析、阈值逻辑、精度、折现边界、Markov/PSM、用户项目或数据库。没有隐式币种转换、价格调整、地区或视角默认值；经济学基础由研究者提供。旧 0.1 的数值与 engine 版本保持可重放，但因缺少基础不得进入后续正式报告合同。
- 验收结果：失败测试转绿；决策树核心 12/12、核心 Skill 11/11、桌面决策树 4/4；全量前端 118 文件/797 项、HEOR 190/190、Rust 378 通过/1 项既有公网测试忽略及资源暂存 2/2、开发合同 400/400、发布合同 62/62、产品文档 15/15，类型检查、ESLint、生产构建、`git diff --check` 和 41 来源/450 文件资源预检全部通过。
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据迁移；旧计划继续由兼容路径读取。决策树 DSA/PSA、专用报告、复现包和亚组分析仍未完成，必须分别建立人工核算案例和失败测试；当前源码尚未重建 DMG，已有 DMG 不包含本次修复。

## Loop P1-SCI-001d：决策树单因素与概率敏感性分析

- 当前行为与目标：当前决策树只能重放基准分析；既有不确定性引擎绑定 Markov 输入，不能安全套用。目标仅为当前决策树 schema 0.2 增加可人工核算、可精确重放的 DSA/PSA；历史 0.1 继续只能探索性重放，不自动升级。
- 修改前失败测试与根因：新增测试首先因 `heor_core.decision_tree_uncertainty` 不存在而失败；第一方运行器、Skill 合同和桌面复核也分别缺少不确定性计划、结果及当前性校验路径。根因是决策树基准分析先于专用不确定性合同交付，而 Markov 参数、周期和状态占用语义与短期事件树不同，不能复用旧引擎而不改变科学含义。
- 最小修复：新增独立不确定性 schema/engine 0.1，只接受当前决策树 0.2、精确基准计划 SHA-256、显式 WTP、显式参数范围、分布、依据标识、PCG32 安全整数种子、迭代数和收敛阈值。可变参数仅限二叉机会节点的一条分支概率及其具名互补分支、终末成本和终末 QALY；禁止重叠目标、隐式归一化、默认分布和推断范围。运行器原子写入 `heor/results/decision-tree-uncertainty.json`，Skill、CLI、验证器和资源启动审计同步纳入。桌面只读面板仅在基准结果与不确定性结果分别绑定当前计划和当前不确定性计划时显示摘要；任一文件缺失、损坏或过期即隐藏数值。
- 科学验收：固定案例在分支概率 0.5 时人工结果为成本 3600、QALY 0.625；在 0.9 时为成本 2480、QALY 0.805、相对比较策略 ICER 5440。每个 PSA 样本均用独立公式 `cost = 5000 - 2800p`、`QALY = 0.4 + 0.45p` 复核。概率为唯一最优和并列最优分开记录；收敛检查点必须递增并以总迭代数结束，显式核对最大概率 MCSE 和检查点漂移。结果保留每次抽样、参数值、成本、QALY 和 NMB；通过只表示所声明参数不确定性按合同执行，不表示模型、证据或结论已经验证。
- 验收结果：HEOR 193/193、核心 Skill 11/11、第一方运行器 5/5、前端 118 文件/798 项、Rust 378 通过/1 项既有公网测试忽略及资源暂存 2/2、开发合同 401/401、发布合同 62/62，类型检查、ESLint、Rust 格式、生产构建、`git diff --check` 和 42 来源/453 文件资源预检全部通过。首次资源测试准确拦截新增 Python 模块未进入 Tauri 映射；补齐资源映射和启动审计后转绿，没有放宽检查。浏览器实渲染确认 `/heor` 可加载并可进入“插件与技能”，无新增控制台错误；纯浏览器环境不伪造 Tauri 本地工件，不确定性双哈希显示路径由组件测试覆盖。
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据库或历史研究文件迁移；Markov/PSM 不确定性路径未改。当前只支持上述三类独立参数，不支持相关参数、结构不确定性、EVPI/EVPPI、多项机会节点概率联动或亚组分析；报告包和复现绑定仍待后续独立 loop。当前源码尚未重建 DMG，已有 DMG 不包含本次修复。

## Loop P1-SCI-001e：决策树专属报告合同与草稿边界

- 当前行为与目标：既有报告包只支持 Markov/PSM 绑定图，无法正确表达短期决策树及其专属 DSA/PSA；直接套用会漏掉决策树计划、不确定性计划和两个确定性结果。目标是在不改变任何计算公式、默认参数、Markov/PSM 合同或历史报告的前提下，建立决策树专属报告 schema，并使科学条件尚不完整的包只能作为研究者可审阅草稿。
- 修改前失败测试与根因：Python 合同首先失败于缺少 `DECISION_TREE_BINDINGS`；Rust 因缺少决策树条目、绑定和 `draft_only_reasons` 失败；第一方 Skill 合同因缺少专属模板失败；前端因报告审计类型与草稿状态不存在失败。根因是决策树基准与不确定性引擎晚于通用报告合同交付，旧固定绑定不能动态选择分析类型。
- 最小修复：新增报告 schema `0.3.0`，仅用于 `analysis_type=decision_tree`，精确绑定报告正文、证据综合、决策树计划、不确定性计划、基准结果和不确定性结果。报告摘要只复制绑定结果中的经济学基础、策略结果、增量前沿、DSA 与 PSA 汇总；逐次 PSA 样本仍由不确定性结果哈希绑定，不复制到摘要。每个 `source_id` 必须解析到同一绑定证据综合中的具体 extraction。报告矩阵只使用 CHEERS 2022 的 28 项，不为决策树伪造 BIA。
- 草稿与 Human 边界：证据综合未到研究者复核状态、参考案例非 current、存在 proposed assumption 或 PSA 未收敛时，报告必须保持 `draft`。结构完整的草稿可导出供研究者审阅，但 `releasable=false`，不得进入发布审阅；只有无草稿原因且具名 release owner 的 `ready_for_release_review` 才可进入下一门禁。结构/哈希错误仍失败关闭，不能以草稿绕过。
- Skill 影响：同步更新第一方 `$heor-reporting` 的短流程、详细合同、确定性验证脚本和专属模板；按 Skill Creator 的 progressive-disclosure 结构保留 `SKILL.md` 简洁，将 schema 细节留在 reference 和 asset，并通过官方 `quick_validate.py` 校验。
- 验收结果：定向 Python 16/16、Rust 报告 11/11、前端复核面板 29/29；全量前端 118 文件/799 项、HEOR 193/193、Rust 380 通过/1 项既有公网测试忽略及资源暂存 2/2、开发合同 406/406、发布合同 62/62；类型检查、ESLint、Rust 格式、生产构建、`git diff --check`、Skill 结构校验和 42 来源/454 文件资源预检全部通过。既有 React `act(...)`、Router、Vite 大包、3Dmol `eval` 与 Node `url.parse` 告警未隐藏，也非本轮引入。
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据库、用户项目或历史报告迁移；旧 Markov/PSM schema 0.1/0.2 保持原行为。当前报告包证明结构、来源与结果摘要一致，不等于研究结论正确或 Human 已批准；决策树报告图尚未进入复现包，亚组分析尚未实现。本轮不生成安装包。

## Loop P1-SCI-001f：决策树可重复性材料绑定与草稿边界

- 当前行为与目标：既有 `heor/reproducibility-package.json` 只接受 Markov/PSM 的三次运行、三项展示和七项 CHEERS/BIA 主张；决策树报告即使结构完整也会被错误要求绑定不存在的 Markov、BIA 和预算影响文件。目标是在不改变旧 schema、不削弱发布门禁且不复制受限资料的前提下，精确绑定决策树报告图、确定性运行方法和具体证据记录。
- 修改前失败测试与根因：便携验证器首先因缺少决策树 `required_claims` 分支失败；Rust 审计首先尝试读取 Markov `uncertainty_plan`；前端把结构完整的决策树复现草稿显示成“不完整”；Skill 合同没有专属模板。根因是复现包合同早于决策树引擎与报告合同建立，固定数量和固定文件路径无法表达新的分析类型。
- 最小修复：新增只用于决策树的 schema `0.2.0`，精确绑定报告包及六个已声明文件，固定两条命令数组（基准计算、决策树不确定性）、两项展示和三项 CHEERS 主张。计划内实际使用的每个 `source_id` 必须先解析到绑定证据综合的 extraction，再解析到具体文献记录；来源可得性仍逐项声明，不抓取、不复制、也不把公开定位符当作内容真实性。Markov/PSM schema `0.1.0`、三次运行和七项主张保持原样。
- 草稿与 Human 边界：报告仍为审阅草稿时，可重复性材料可以结构完整，但状态必须为 `draft`、`release_companion_ready=false`。界面单独显示待解决的假设、证据综合、参考案例或收敛原因，不再把草稿误报为结构缺失。该合同只验证当前字节、哈希、运行配方和追溯关系；不证明来源真实、科学有效、外部环境已复现或 Human 已批准。
- Skill 影响：同步更新第一方 `$heor-reproducibility-package` 的短流程、详细合同、专属模板和便携验证器；通过 Skill Creator 官方结构校验。详细 schema 留在 reference/asset，避免将大量实现细节塞入 Skill 主文件。
- 验收结果：决策树复现合同 8/8、Rust 复现审计 6/6、前端草稿边界定向测试通过；全量前端 118 文件/800 项、HEOR 193/193、Rust 383 通过/1 项既有公网测试忽略、开发合同 409/409、发布合同 62/62，类型检查、ESLint、Rust 格式、生产构建、`git diff --check`、Skill 结构校验和 42 来源/455 文件资源预检全部通过。资源预检首次准确拦截本轮测试生成的 `__pycache__`，删除该生成缓存后通过，未放宽规则。
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据库或历史研究文件迁移。决策树现已能生成和审计可重复性材料，但既有 Human 发布执行仍绑定 Markov/PSM 独立验证与重跑路径；本轮未绕过该门禁，也未宣称决策树可正式发布。决策树专属独立验证/发布重跑和亚组分析仍需分别建立失败测试与人工核算基准。本轮不生成安装包。

## Loop P1-SCI-002：来源绑定的受限决策树亚组分析

- 当前行为与目标：决策树已有当前基准分析、DSA/PSA、报告与复现材料，但“亚组分析”只有叙述性建议，不能把人群定义、占比、各亚组输入和结果绑定到同一证据图，也不能防止事后分组被误读为交互作用。目标是建立一个只适用于当前短期决策树的描述性、可人工核算、可精确重放合同。
- 修改前失败测试与根因：核心测试首先因 `heor_core.subgroup_analysis` 不存在而失败；第一方运行器不识别 `--subgroup-plan`；桌面复核没有亚组状态；资源合同准确拦截新增 Python 模块未进入 Tauri 包。根因是此前各决策树能力围绕总体人群设计，未建立亚组人口权重、可比性、来源解析和科学解释边界。
- 最小修复：新增独立 schema/engine 0.1，要求 2–20 个互斥且穷尽的亚组、占比和为 1、预设或事后状态、完整来源/拟议假设、与总体模型一致的策略、基线、经济学基础、参考案例、时域、折现、半周期设置和阈值。每个亚组是 `heor/subgroups/` 下的完整当前决策树，并由路径与 SHA-256 精确绑定；所有来源必须解析到绑定证据综合中的具体 extraction 和文献记录。引擎独立运行各亚组、按占比加权，并分别核对成本与 QALY 相对总体模型的差异，不混合不同单位的误差。
- 科学与 Human 边界：人工固定案例得到 A 组增量成本/QALY 680/0.125、INMB 5570，B 组 1520/-0.01、INMB -2020，按 50%/50% 加权为 1100/0.0575、INMB 1775，与总体模型精确一致；组间差异仅标记为描述性。六项人口定义、预设状态、来源适用性、异质性依据、多重性/效能和解释用途始终等待研究者复核；系统不执行交互检验、不发现亚组、不生成亚组决策规则，也不创建虚假批准。
- 产品与溯源：第一方运行器只在完整图通过后原子写入独立亚组结果；桌面仅显示与当前总体计划、亚组计划、证据综合和每个亚组输入字节一致的结果。亚组数字可点击打开对应的具体亚组输入，计划与结果均可单独打开；任一来源未登记、文件损坏或哈希过期即隐藏数字并要求重跑。新模块进入 Tauri 资源映射和启动审计，`$heor-decision-tree` 与 `$heor-workbench` 均记录同一边界。
- 验收结果：新增亚组核心 4/4、第一方运行器、核心 Skill、桌面复核、启动资源和 Skill Creator 校验全部通过；全量前端 311 文件/801 项、HEOR 197/197、Rust 383 通过/1 项既有公网测试忽略、开发合同 411/411、发布合同 62/62，类型检查、ESLint、Rust 格式、生产构建、`git diff --check` 和 43 来源/459 文件资源预检通过。既有 React `act(...)`、Router、Vite 大包、3Dmol `eval`、SWIG 与 Node `url.parse` 告警未隐藏，也非本轮引入。
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据库、历史研究或既有 Markov/PSM schema 迁移；没有亚组文件时既有行为不变。当前亚组合同不支持数据驱动发现、连续效应修饰、IPD 回归、亚组 NMA/MAIC、因果森林、多重性校正或自动结论。该 loop 完成时尚缺的决策树独立验证与发布门禁已由后续 P1-SCI-001g 独立闭环。本轮不生成安装包。

## Loop P1-SCI-001g：决策树专属独立验证与确定性发布重放

- 当前行为与目标：决策树计算、DSA/PSA、报告和复现包已有独立工件图，但 Human 发布执行仍固定要求“决策问题→概念模型→分析计划→独立验证→发布”的 Markov/PSM 顺序，并固定重放 Markov、PSA 与 BIA。目标是在不降低既有门禁的前提下，为决策树建立有实际责任对象的“独立验证→发布”流程，且发布前重新执行当前决策树基准与不确定性计算。
- 修改前失败测试与根因：新增审批顺序测试首先因缺少三项 Markov 前置批准而失败；第一方 Skill 合同因缺少决策树验证模板失败；前端类型与 ESLint 随后准确拦截了可空 Markov 计划和新字面值。根因是审批、模型验证和发布重跑都早于决策树工件图建立，固定假设当前研究必有 Markov 概念模型、Markov 分析计划和 BIA。
- 最小修复：新增模型验证 schema `0.3.0`，精确绑定当前证据综合、决策树计划、决策树不确定性计划及两项确定性结果，只覆盖成本效果验证，不创建 BIA 路径。验证覆盖表面、输入、外部、交叉、预测及 TECH-VER 五类计算组件；外部效度必须通过，交叉/预测无法执行时仍须以本地证据和理由记录。桌面仅显示独立验证与发布两张责任卡；Markov/PSM 原五关流程保持不变。
- 发布与重放：独立验证事件绑定上述五个当前工件及验证报告，审阅者姓名必须匹配报告；发布事件必须由报告中的具名发布责任人执行，并绑定报告、复现包和当前工件。发布检查从打包的第一方引擎重新执行基准与不确定性计算，比较完整 JSON 值，并继续以已落盘原始字节 SHA-256 绑定工件；这避免 Python/Rust 对科学计数法格式差异造成误阻断，同时任何数值、结构或绑定变化仍失败关闭。撤销独立验证会使发布立即失效，并保留决策树路由身份。
- 科学与 Human 边界：结构、哈希、覆盖和重放通过不等于科学结论正确。独立审阅者身份、独立性、利益冲突、观察和建议必须由具名合格人员填写；发布责任人另行确认发布。旧决策树验证 schema `0.1.0` 不能进入正式关卡；历史 Markov/PSM 工件、批准日志和计算公式不迁移、不改写。
- Skill 与多语言：`$heor-model-validation` 增加专属模板、合同和便携验证分支；中英文任务提示明确选择 schema 0.3 和五项绑定，七种已发布界面的 Skill 目录名称与描述继续由既有完整性门禁校验。Skill Creator 官方校验通过。
- 验收结果：定向决策树 Rust 12 项、便携验证合同 9/9、前端专用复核 7/7 及 Skill 结构校验通过；全量前端 311 文件/802 项、HEOR 197/197、Rust 390 通过/1 项既有公网测试忽略及资源暂存 2/2、开发合同 414/414、发布合同 62/62，类型、ESLint、Rust 格式、生产构建、差异检查和 43 来源/460 文件资源预检全部通过。根级原生 E2E 重建测试特性二进制，并在真实 Tauri/WKWebView 中通过既有任务、队列、Human 输入、权限、provider 失败恢复、任务文件、被动 HTML、项目导入与确定性 DOCX/PDF/XLSX 导出分支。
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据库、用户项目、历史审批或科学结果迁移。原生 E2E 证明当前源码调试构建无回归，但尚未用当前源码的新 DMG 生成安装后字节级发布证据；必须在干净提交后完成最终 macOS 1.0.0 打包与安装后验证。本轮不生成安装包。
