...
| P1-SCI-001 | P1 | 决策树经济学基础、DSA/PSA、报告和可重复性材料合同均已完成；发布阶段的专属独立验证仍须单独闭环 | 当前 0.2 计划必须声明币种、价格年份、地区和分析视角，结果精确回传同一基础；专属不确定性引擎、报告 schema 0.3 和可重复性 schema 0.2 均精确绑定当前决策树图，草稿不得冒充可发布。旧 0.1 仅兼容重放并明确标为探索性。现有 Human 发布门禁仍依赖 Markov/PSM 独立验证合同，决策树不得绕过，需后续建立专属验证与发布重跑合同 |
...
9. P1-SCI-001：决策树 0.2 经济学基础、DSA/PSA、报告与复现包已完成；P1-SCI-002 的受限描述性亚组分析也已用人工核算案例独立完成。下一项是决策树专属独立验证与发布重放，现有 Markov/PSM 门禁不得绕过。
...
- 回滚与剩余风险：回退本 loop 独立提交即可，无数据库、历史研究或既有 Markov/PSM schema 迁移；没有亚组文件时既有行为不变。当前亚组合同不支持数据驱动发现、连续效应修饰、IPD 回归、亚组 NMA/MAIC、因果森林、多重性校正或自动结论。决策树仍不能通过现有 Markov/PSM 独立验证发布门禁；该项继续作为下一独立 P1。本轮不生成安装包。
