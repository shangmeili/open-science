...
| P1-TEST-001b | P1 | 调试构建的主要交互分支和安装包内 A1 权限持久化已完成；安装后完整流程待后续 loop | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界、任务作用域隔离、队列排序/删除/逐条发送、OpenCode `question` 回答恢复、真实 provider 400 错误后的同任务执行器清理与单条队列恢复，以及默认“由我确认”模式下危险 bash 命令的一次授权、明确拒绝、工具结果和队列恢复；当前 DMG 又验证了精确项目/动作/资源规则的保存、sidecar 重启复用、撤销和重新询问。安装包完整任务 UI 和导入到导出仍缺自动化 E2E |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML、项目/独立任务作用域、消息队列、Human 问题恢复、真实 provider 失败恢复及危险命令一次授权/拒绝 E2E 已完成；当前安装包也已验证“始终允许”的精确作用域、重启复用、撤销和重新询问。后续分别补安装包完整任务 UI 和导入到导出，不用一个过大 E2E 隐藏失败点。
...
9. P1-SCI-001：首版确定性内核已完成；后续分别为 Skill、桌面审查、报告与复现包建立独立合同。P1-SCI-002 仍需先建立人工可核算基准，禁止与界面修复混做。
10. P0-AI-002：产品 Harness 已改为每次运行必定校验和叠加加载，用户/项目指令保留但不得替代产品治理边界（已完成）。
11. P1-AI-001a、P1-AI-001b1、P1-AI-001b2 和 P1-AI-001b3：内容无关模型调用账本、固定 HEOR 前导提示指纹、当次实际最终系统块指纹及模型调用到具体工具产物/运行的准确关联已完成；P1-AI-001c 单独建立研究者可见审计界面，不以内部标识符替代可解释产品界面。
12. P2-SEC-003：评估主应用全局 CSP；仅在不破坏本地服务和模型流式连接时实施。
13. P1-LEGAL-001：依赖锁文件变更后必须重生成许可证清单，其锁文件 SHA-256 不一致时由现有打包资源合同 fail-closed（已完成）。
