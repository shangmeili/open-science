...
- P1-AI-001b2 开始前的已提交基线：`38babd9`；P1-AI-001b3 实施前的已提交基线：`bc71b1b`；P1-TEST-001b1 实施前的已提交基线：`38a5687`；P1-TEST-001b2 实施前的已提交基线：`076180f`；P1-TEST-001b3 实施前的已提交基线：`14254f8`。更早已完成业务修复均保留为独立提交，本轮不重复修改。
...
| P1-TEST-001b | P1 | 调试构建的导航、被动 HTML 与独立任务隔离已完成；安装后完整流程待后续 loop | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界，以及本地夹具模型下项目与独立任务作用域隔离；安装包任务执行、权限和导入到导出仍缺自动化 E2E |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML，以及真实项目创建、独立任务创建和作用域隔离 E2E 已完成；后续分别补安装包任务执行、权限与导入到导出，不用一个过大 E2E 隐藏失败点。
