...
- P1-AI-001b2 开始前的已提交基线：`38babd9`；P1-AI-001b3 实施前的已提交基线：`bc71b1b`；P1-TEST-001b1 实施前的已提交基线：`38a5687`；P1-TEST-001b2 实施前的已提交基线：`076180f`；P1-TEST-001b3 实施前的已提交基线：`14254f8`；P1-TEST-001b4 实施前的已提交基线：`1cf99a8`；P1-TEST-001b5 实施前的已提交基线：`6f48f57`；P1-TEST-001b6 实施前的已提交基线：`93912be`。更早已完成业务修复均保留为独立提交，本轮不重复修改。
...
| P1-TEST-001b | P1 | 调试构建的导航、被动 HTML、独立任务隔离、消息队列、Human 问题恢复与危险命令权限恢复已完成；安装后完整流程待后续 loop | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界、任务作用域隔离、队列排序/删除/逐条发送、OpenCode `question` 回答恢复，以及默认“由我确认”模式下危险 bash 命令的具体资源展示、一次授权、工具结果和队列恢复；安装包完整任务、拒绝/永久允许分支、失败恢复和导入到导出仍缺自动化 E2E |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML、项目/独立任务作用域、消息队列、Human 问题恢复及危险命令一次授权 E2E 已完成；后续分别补安装包完整任务、权限拒绝/永久允许、provider 失败恢复与导入到导出，不用一个过大 E2E 隐藏失败点。
