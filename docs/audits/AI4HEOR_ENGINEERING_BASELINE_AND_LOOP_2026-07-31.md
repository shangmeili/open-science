...
当前仓库已有主要单元/组件、确定性 HEOR、Rust 原生与资源门禁测试。macOS 与 Windows 发布脚本已有真实主机级安装、进程、工作区和清理检查，并已补上 OpenCode 鉴权 HTTP 与前端 bootstrap 合同。测试专用 macOS WebDriver 变体已在真实 Tauri/WKWebView 中完成基础导航、被动 HTML 预览、项目/独立任务作用域、队列、Human 问题、权限、provider 失败恢复，以及真实项目导入—项目内任务执行—确定性 DOCX/PDF/XLSX 报告导出验证。macOS DMG 验证器现另用发布二进制外部的 Accessibility 语义驱动按精确 PID 验证窗口、新建任务、可编辑输入框、任务文件和插件与技能入口，不把 WebDriver 编进正式应用；当前源码尚未重新生成 DMG，安装包内模型回复链也仍待独立验收。
...
| P1-TEST-001b | P1 | 调试构建主要交互分支及导入—执行—导出已完成；安装包内 A1 权限已验证；安装包导航与输入驱动已实现，完整回复链仍待后续 loop | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界、任务作用域隔离、队列排序/删除/逐条发送、OpenCode `question` 回答恢复、provider 400 失败恢复、危险 bash 命令的一次授权/拒绝，以及项目原生导入、受管副本内项目任务执行、确定性 DOCX/PDF/XLSX 报告导出与当前性审计；外部源目录保持不变。DMG 验证器现从外部按精确 PID 驱动正式应用的语义辅助功能树，现有安装版隔离副本已通过窗口、新建任务、输入框、任务文件和插件与技能冒烟；当前源码 DMG 与安装包内本地 provider 回复仍待实证 |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML、项目/独立任务作用域、消息队列、Human 问题恢复、provider 失败恢复、危险命令一次授权/拒绝，以及项目导入—项目内任务—确定性报告导出 E2E 已完成；当前安装包也已验证“始终允许”的精确作用域、重启复用、撤销和重新询问。外部 Accessibility 驱动现可在不修改发布二进制的前提下验证 DMG 应用的窗口、任务入口、可编辑输入框和核心导航；后续以独立 loop 接入本地 provider 回复，不用一个过大 E2E 隐藏失败点。
