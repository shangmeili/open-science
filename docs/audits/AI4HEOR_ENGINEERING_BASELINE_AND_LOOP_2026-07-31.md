...
| P1-TEST-001b | P1 | 调试构建的主要交互分支和安装包内 A1 权限持久化已完成；安装后完整流程待后续 loop | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界、任务作用域隔离、队列排序/删除/逐条发送、OpenCode `question` 回答恢复，以及默认“由我确认”模式下危险 bash 命令的一次授权、明确拒绝、工具结果和队列恢复；当前 DMG 又验证了精确项目/动作/资源规则的保存、sidecar 重启复用、撤销和重新询问。安装包完整任务 UI、provider 失败恢复和导入到导出仍缺自动化 E2E |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML、项目/独立任务作用域、消息队列、Human 问题恢复及危险命令一次授权/拒绝 E2E 已完成；当前安装包也已验证“始终允许”的精确作用域、重启复用、撤销和重新询问。后续分别补安装包完整任务 UI、provider 失败恢复和导入到导出，不用一个过大 E2E 隐藏失败点。
