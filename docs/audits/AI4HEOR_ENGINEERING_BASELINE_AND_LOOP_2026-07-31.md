...
当前仓库已有主要单元/组件、确定性 HEOR、Rust 原生与资源门禁测试。macOS 与 Windows 发布脚本已有真实主机级安装、进程、工作区和清理检查，并已补上 OpenCode 鉴权 HTTP 与前端 bootstrap 合同。测试专用 macOS WebDriver 变体已在真实 Tauri/WKWebView 中完成基础导航、被动 HTML 预览，以及本地夹具模型驱动的真实项目创建、独立任务创建和项目隔离验证；安装后任务执行、权限交互和导入到导出 E2E 仍未建立，且调试构建证据不能替代安装包字节的原生验收。
...
| P1-TEST-001b | P1 | 调试构建的导航、被动 HTML、独立任务隔离、消息队列、Human 问题恢复与危险命令允许/拒绝分支已完成；安装后完整流程待后续 loop | 测试专用 macOS Tauri WebDriver 已验证 IPC 桥接、真实侧边栏/文件操作、HTML 安全边界、任务作用域隔离、队列排序/删除/逐条发送、OpenCode `question` 回答恢复，以及默认“由我确认”模式下危险 bash 命令的具体资源展示、一次授权、明确拒绝、工具结果和队列恢复；安装包完整任务、永久允许的持久化/作用域、失败恢复和导入到导出仍缺自动化 E2E |
...
3. P1-TEST-001b：调试构建的基础导航、被动 HTML、项目/独立任务作用域、消息队列、Human 问题恢复及危险命令一次授权/拒绝 E2E 已完成；后续分别补安装包完整任务、永久允许的持久化与作用域、provider 失败恢复和导入到导出，不用一个过大 E2E 隐藏失败点。
...
- 上游依据与根因：固定 OpenCode `1.17.13` 的 [bash 工具](https://github.com/anomalyco/opencode/blob/10c894bdeef3618f5666fb506ef7f9491bb964d8/packages/core/src/tool/bash.ts) 确认输入为 `command`/可选 `workdir`/可选 `timeout`，执行前对 `bash` 资源进行权限断言；[权限服务](https://github.com/anomalyco/opencode/blob/10c894bdeef3618f5666fb506ef7f9491bb964d8/packages/core/src/permission.ts) 确认 `once` 只解锁当前请求。缺口的根因不是产品逻辑缺失，而是本地 provider 夹具无法可重复地生成 `bash` `tool_use`。
...
- 上游依据与根因：固定 OpenCode `1.17.13` 的[权限服务](https://github.com/anomalyco/opencode/blob/10c894bdeef3618f5666fb506ef7f9491bb964d8/packages/core/src/permission.ts)在 `reject` 时拒绝当前 deferred 权限并结束工具回合。缺口根因是本地 provider 夹具只有一个一次授权 bash 请求，无法用不同工具 ID 和实际存在的文件区分“拒绝”与“命令已执行”。
