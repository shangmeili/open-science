...
- 本轮开始前的已提交基线：`d14ad63`；更早已完成业务修复均保留为独立提交，本轮不重复修改。
...
| AI 调用与任务执行 | OpenCode 本地 sidecar，HTTP/SSE，模型提供商可配置 | 完成调用的提供商、模型、时间、token、缓存 token、结束原因和运行时报告费用已归一化；新的 HEOR 调用记录固定前导提示的精确 SHA-256 与回复语言；sidecar 每次启动都校验并额外加载应用自有产品 Harness，项目原有指令文件不被覆盖 |
...
| P1-AI-001b2 | P1 | 调查完成，待人工决策 | P0-AI-002 已保证经校验的产品 Harness 必定叠加加载；OpenCode 1.17.13 现有插件钩子不能把最终系统指令集合可靠绑定到具体助手消息，精确实现需要采用经审查的自带 OpenCode 最小补丁 |
...
9. P1-AI-001a 和 P1-AI-001b1：不记录密钥、研究者文本或回答正文的模型调用账本，以及应用固定 HEOR 前导提示指纹已完成；P1-AI-001b2/b3 继续分别补齐当次实际指令组合指纹和研究产物关联，P1-AI-001c 单独建立研究者可见审计界面。
...
- 剩余风险：`runtimeReportedCost` 只保留运行时给出的数值，不自行推定币种、账单或结算金额；应用固定 HEOR 前导提示指纹已由 P1-AI-001b1 补齐，实际 Harness、产物级调用关联和面向研究者的审计查看界面分别仍属 P1-AI-001b2/b3/c。账本是完整性检测而非外部可信时间戳或数字签名，不应被描述为不可否认审计。
...
- 剩余风险：该指纹只证明应用固定 HEOR 前导提示，不代表当次实际工作区 Harness，也没有建立调用到产物的因果或来源关联；已存在且未记录上下文的历史账本不会被追溯改写。分别保留为 P1-AI-001b2、P1-AI-001b3 和 P1-AI-001c。
...
- 剩余风险：上游组织/受管配置的更后置合并层级未在本地企业环境实证，不扩大宣称为所有上游扩展下的绝对最终优先级；原生工具权限仍是强制执行边界。P1-AI-001b2 仍需记录当次实际同时加载的用户/项目指令组合指纹，不得把本轮修复误报为调用审计已完成。

## Loop P1-AI-001b2 调查与待确认边界
...
- 当前状态：调查与实现边界已完成，业务代码、OpenCode 二进制、账本 schema 和研究数据均未修改；等待研究者确认方案 A 及本地 SHA-256 派生数据边界。
