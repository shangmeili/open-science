...
## Auditable public evidence search
...
app-owned connector. The desktop audits the exact bytes and exposes the query,
dates, fixed source allowlist, per-source cap, non-sensitive egress declaration,
and SHA-256. Confirmation mode asks once before network egress. User-selected
full-access mode proceeds without a second prompt and records the runtime policy,
not a human scientific approval.
...
app-owned storage outside the Agent workspace. A changed request creates a new
execution record.
