...
2. Confirm current analysis schema `0.15.0`, PSM schema `0.7.0`, treatment-duration schema `0.1.0`, states exactly `progression_free`, `progressed`, `dead`, and one common analysis time grid. Create joint manifest `0.5.0`; prior-current manifests `0.4.0` and `0.3.0` remain readable. Legacy analysis `0.12.0` / PSM `0.3.0` or `0.4.0` with manifest `0.1.0` or `0.2.0` also remains readable.
3. Confirm there is already either a reviewed joint posterior or a complete `$heor-paired-survival-bootstrap` result that covers every strategy PFS and OS endpoint. A paired-bootstrap source additionally requires the current exact app-owned Human acceptance record from the AI4HEOR review pane. This Skill audits and packages those draws; it does not fit survival models, reconstruct covariance from marginal intervals, or create/edit a method-review record.
...
5. Copy [assets/joint-survival-uncertainty.template.json](assets/joint-survival-uncertainty.template.json) to `heor/joint-survival-uncertainty.json`. Bind the exact analysis, PSM, source materialization, treatment-duration, draw-file, and source-artifact bytes by lowercase SHA-256. For `paired_patient_bootstrap`, also copy the app record's declared review identity, action, assurance, result binding, path, and hash into `generation.method_review`; bind that same record in `source_artifacts` without changing its bytes.
...
- Do not create or edit paired-bootstrap method-review records, create approval events, accept assumptions, or claim independent fitting validation. The Human reviews the method and evidence through the native app; the Rust audit independently replays the plan and curve calculations but does not refit source data.
