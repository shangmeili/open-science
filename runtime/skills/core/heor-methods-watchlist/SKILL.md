...
6. When a revision is suspected or confirmed, record the changed sections, affected AI4HEOR contracts, required Human actions, and preparation status. Do not silently rewrite downstream artifacts.
7. Run the validator and present overdue checks, unresolved changes, and limitations in plain language. Every change remains unresolved in portable output.
8. Route the researcher to the desktop's auxiliary Human review control to record `accept_revalidation` or `dismiss_change`. Never write, imitate, or infer that app-owned decision in the workspace artifact.
...
A valid `draft` may still be incomplete. With no changes, `ready_for_human_review` is portable-complete only when the artifact is structurally valid and every check is current as of `as_of_date`. With changes, only the native app can report complete after verifying exact-byte-bound Human review events.
...
- Do not add `human_disposition`, approval, reviewer, or acceptance fields to Agent-writable artifacts.
