...
3. Create analysis schema `0.14.0` by retaining the cost link and adding only `"utility_inputs": {"path": "heor/utility-inputs.json"}`. Keep `state_utilities` equal to the first cycle of the declared schedule.
...
7. Keep acute or recurrent event disutilities outside this health-state schedule. If overlap cannot be assessed, stop and request Human resolution rather than risking double counting.
...
- Do not add event disutilities, treatment-related effects, caregiver effects, or component uncertainty to calculations unless a later dedicated contract explicitly supports them.
