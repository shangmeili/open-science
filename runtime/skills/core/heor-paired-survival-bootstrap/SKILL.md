...
10. Leave the result `awaiting_bootstrap_method_review` and ask the researcher to open the AI4HEOR review pane. The native app independently replays the resampling plan and every curve/coherence check. Only the Human may accept or reject the exact result through the seven-item method checklist; the app writes the review record and its separate hash-chained event. Never create, edit, imitate, or infer this record.
11. Only after app-owned acceptance, use `$heor-joint-survival-uncertainty` schema `0.5.0` to bind the exact execution, candidate bytes, and accepted method-review record into canonical joint-survival artifacts. Do not copy candidate draws into PSA directly.
...
- Do not select curve families, suppress failed replicates, create or edit method-review records or approval events, claim independent validation, or call candidate output decision-ready.

## Handoff

Report request, source, plan, replicate, candidate, adapter, evaluator, runtime and result hashes; arm counts; selected family order; seed; iteration and failure counts; portable and native numerical/PFS/OS coherence status; dependence preserved; conditional-independence assumption; limitations; validator result; and whether the current app-owned Human review is absent, rejected, accepted, or stale.
