...
The request stays `awaiting_execution_authorization`. Run only after a Human authorizes the exact local command, current source classification, reviewed curve families, and parallel-arm resampling design. The result never records an approval event and requires a separate Human method review before canonical joint-survival packaging.
...
After portable execution audit, a Human reviews the resampling unit, endpoint definitions, censoring, selected families, convergence/failure profile, follow-up and extrapolation, parallel-arm independence assumption, and clinical plausibility. Only then may `$heor-joint-survival-uncertainty` bind the exact execution manifest, replicate file, candidate draws, and current analysis artifacts into the canonical joint-survival manifest; Python and native Rust independently audit that canonical artifact before PSA consumption.
