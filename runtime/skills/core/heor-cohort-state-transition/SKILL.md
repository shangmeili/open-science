...
3. Use one static `transition_matrix` per strategy when probabilities do not change over model cycle. Use a `transition_schedule` only when evidence or an explicit proposed assumption supports model-cycle-dependent matrices. Use schema `0.8.0` for ordinary multi-strategy analysis, `0.9.0` only for background mortality, `0.10.0` only for the admitted RR/OR relative-effect transformation, and `0.11.0` only for the admitted constant-HR transformation. Preserve schemas `0.4.0`–`0.7.0` for compatible legacy projects.
