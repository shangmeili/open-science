...
7. Use `$heor-uncertainty-analysis` for evidence-supported uncertainty. Sample a directly modeled complete probability row with Dirichlet. For schema `0.5.0` rate-derived transitions, vary only exact positive event-rate targets through uncertainty schema `0.4.0` so the engine recomputes the complete transformation; correlate only evidence-bound lognormal members through the bounded Cholesky contract. Test schema `0.4.0` schedule change points only as declared structural scenarios.
