...
3. Use one static `transition_matrix` per strategy when probabilities do not change over model cycle. Use a `transition_schedule` only when evidence or an explicit proposed assumption supports model-cycle-dependent matrices; use schema `0.5.0` when `$heor-transition-rate-adapter` derives transitions, schema `0.6.0` when `$heor-survival-curve-adapter` derives the admitted two-state schedule, schema `0.7.0` when `$heor-probability-time-adapter` derives bounded single-event transitions, and otherwise use `0.4.0`.
...
6. Map the complete static matrix or schedule through `$heor-input-provenance`. Use `$heor-transition-rate-adapter` only for bounded constant competing rates, `$heor-survival-curve-adapter` only for its already-selected two-state exponential or Weibull case, and `$heor-probability-time-adapter` only when every row has at most one event with an explicit source interval and reviewable constant-hazard assumption. Do not assemble probabilities, perform broader rate or hazard conversions, pool sources, fit or select curves, extrapolate treatment effects, or normalize competing risks through prose; leave unsupported derivations incomplete.
7. Use `$heor-uncertainty-analysis` for evidence-supported uncertainty. Sample a directly modeled complete probability row with Dirichlet. For analysis schema `0.5.0` rate-derived transitions, vary only exact positive event-rate targets; for analysis schema `0.6.0` survival schedules, use uncertainty schema `0.5.0` and vary only exact positive curve-parameter values; for analysis schema `0.7.0`, use uncertainty schema `0.6.0` and vary only the exact source probability with Beta or bounded Uniform. The engine must recompute the complete affected transformation. Correlate only evidence-bound lognormal members through the bounded Cholesky contract. Test directly declared schedule change points only as structural scenarios.
...
- Never label a model-cycle schedule as time-in-state, tunnel-state, semi-Markov, or individual simulation. A schema `0.6.0` schedule may evaluate a declared survival curve, and a schema `0.7.0` schedule may convert a declared source probability under constant hazard; neither establishes clinical validity.
