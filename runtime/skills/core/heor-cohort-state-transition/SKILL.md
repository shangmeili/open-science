...
3. Use one static `transition_matrix` per strategy when probabilities do not change over model cycle. Use schema `0.4.0` `transition_schedule` only when evidence or an explicit proposed assumption supports model-cycle-dependent matrices.
...
6. Map the complete static matrix or schedule through `$heor-input-provenance`. Do not assemble probabilities, convert rates or hazards, pool sources, extrapolate treatment effects, or normalize competing risks through prose; use a bounded deterministic adapter or leave the derivation incomplete.
