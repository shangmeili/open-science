...
3. Use one static `transition_matrix` per strategy when probabilities do not change over model cycle. Use a `transition_schedule` only when evidence or an explicit proposed assumption supports model-cycle-dependent matrices. For a new multi-strategy analysis, use schema `0.8.0` and retain the admitted bounded rate, survival, or probability-time transformation at the relevant dynamic strategy path; preserve schemas `0.4.0`–`0.7.0` for compatible legacy projects.
...
7. Use `$heor-uncertainty-analysis` for evidence-supported uncertainty. Sample a directly modeled complete probability row with Dirichlet. For analysis schema `0.8.0`, use uncertainty schema `0.7.0`; vary only admitted direct rows or exact source rates, curve parameters, or source probabilities, and recompute the complete affected transformation. Correlate only evidence-bound lognormal members through the bounded Cholesky contract. Test directly declared schedule change points only as structural scenarios.
