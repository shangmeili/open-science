...
3. Use one static `transition_matrix` per strategy when probabilities do not change over model cycle. Use a `transition_schedule` only when evidence or an explicit proposed assumption supports model-cycle-dependent matrices. For ordinary new multi-strategy analysis, use schema `0.8.0`; use schema `0.9.0` only for the admitted background-mortality transformation. Preserve schemas `0.4.0`–`0.7.0` for compatible legacy projects.
...
6. Map the complete static matrix or schedule through `$heor-input-provenance`. Use `$heor-transition-rate-adapter` only for bounded constant competing rates, `$heor-survival-curve-adapter` only for its already-selected two-state exponential or Weibull case, `$heor-probability-time-adapter` only when every row has at most one event with an explicit source interval and reviewable constant-hazard assumption, and `$heor-background-mortality` only for its two-state annual-probability-to-positive-cycle additive excess-hazard case. Do not assemble probabilities, perform broader rate or hazard conversions, pool sources, fit or select curves, extrapolate treatment effects, or normalize competing risks through prose; leave unsupported derivations incomplete.
7. Use `$heor-uncertainty-analysis` for evidence-supported uncertainty. Sample a directly modeled complete probability row with Dirichlet. Pair analysis `0.8.0` with uncertainty `0.7.0`; pair background-mortality analysis `0.9.0` with uncertainty `0.8.0`, varying only the exact constant excess rate while keeping the age-aligned general-population table fixed. Correlate only evidence-bound lognormal members through the bounded Cholesky contract. Test directly declared schedule change points only as structural scenarios.
