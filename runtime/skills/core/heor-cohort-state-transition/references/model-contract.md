...
Analysis schemas `0.4.0` through `0.11.0` accept exactly one transition mechanism for each strategy:
...
- Schema `0.3.0` remains valid for static matrices. Schedules require schema `0.4.0` or later. Schema `0.8.0` admits the existing bounded transition transformations at dynamic strategy paths and requires 2–16 explicitly ordered strategies. Schema `0.9.0` adds `$heor-background-mortality`; schema `0.10.0` adds the bounded `$heor-relative-effect-adapter`; schema `0.11.0` adds the bounded `$heor-hazard-ratio-adapter`.
...
Map a static matrix at `strategies.<role>.transition_matrix`. Map a schedule at `strategies.<role>.transition_schedule`. The derivation snapshot must equal the complete current value. A direct-evidence extraction must contain strict JSON equal to that complete matrix or schedule. Constant cause-specific competing event rates may be transformed only through `$heor-transition-rate-adapter`; an already-selected exponential or Weibull two-state survival curve may be evaluated only through `$heor-survival-curve-adapter`; a single event probability with an explicit source interval may be converted only through `$heor-probability-time-adapter`; age-aligned annual general-population all-cause probabilities plus one constant additive disease excess hazard may be combined only through `$heor-background-mortality`; aligned cycle-specific baseline risks plus one RR or OR may be transformed only through `$heor-relative-effect-adapter`; cycle-aligned baseline cumulative hazards plus one reviewed constant HR may be transformed only through `$heor-hazard-ratio-adapter`. Other assembly from several estimates remains incomplete.
...
For a schema `0.5.0` transition derived from constant competing rates, do not use these probability-row targets. Uncertainty schemas `0.3.0` through `0.6.0` may instead target an exact positive event `rate_per_year` inside `input_provenance`; the uncertainty engine then recomputes the complete affected matrix or schedule before model validation. Gamma, lognormal, and strictly positive uniform rate distributions are admitted. Schemas `0.4.0` through `0.6.0` additionally admit evidence-bound Cholesky correlation only among lognormal scalar members. For a schema `0.6.0` survival transformation, uncertainty schema `0.5.0` or `0.6.0` may target only the exact positive exponential rate or Weibull shape or scale value and must recompute the complete schedule. For a schema `0.7.0` probability-time transformation, uncertainty schema `0.6.0` may target only its exact source probability with Beta or bounded Uniform and must recompute the complete transition input. For analysis schema `0.9.0`, uncertainty schema `0.8.0` may vary only the exact positive `excess_mortality_rate_per_year.value`. For analysis schema `0.10.0`, uncertainty schema `0.9.0` may vary only `relative_effect.value` under its measure-specific RR/OR support and ceiling. For analysis schema `0.11.0`, uncertainty schema `0.10.0` may vary only `hazard_ratio.value` with strictly positive bounded Uniform support whose high reproduces a finite complete schedule. All routes keep transformation internals fixed and recompute the complete schedule. General CTMC intensity uncertainty, arbitrary or unsupported correlated distributions, curve selection, time-varying or non-proportional HR application, multiplicative/SMR mortality structures, and transformation-space structural scenarios remain unsupported.

## Explicit exclusions

The current bounded contract does not implement tunnel states automatically, time-since-entry transitions, patient-level history, recurrent-event trackers, general continuous-time matrix exponentiation, competing-probability conversion, relative-effect application outside the dedicated RR/OR and constant-HR adapters, treatment-effect extrapolation formulas, dynamic populations, interactions, time-varying rewards, partitioned survival, or microsimulation. The HR route accepts only one reviewed constant HR applied to cycle-aligned baseline cumulative-hazard increments for one absorbing time-to-first event. The background-mortality route additionally stops for already all-cause inputs, cause-specific/subdistribution mixing, calendar mortality improvement, age/sex mixtures, time-varying excess hazards, and competing non-death events. Additive versus multiplicative disease/background mortality can materially differ; this version implements additive excess hazard only, so the untested multiplicative alternative remains a Human-in-the-loop structural limitation. Record everything else as a structural gap; never approximate it invisibly with a model-cycle schedule.
