...
description: Derive, propagate uncertainty through, and audit AI4HEOR schema 0.5.0 cohort transition matrices or model-cycle schedules from constant cause-specific competing event rates. Use when evidence reports rates rather than cycle probabilities, each event can be bound to one extraction or proposed assumption, or an uncertainty schema 0.3.0 plan needs rate-space DSA/PSA; do not use for probability time conversion, relative effects, general CTMCs, extrapolation, or correlated rate models.
---
...
7. When evidence supports rate uncertainty, use `$heor-uncertainty-analysis` schema `0.3.0`. Target only the exact event `rate_per_year`, require positive DSA bounds and gamma, lognormal, or strictly positive uniform PSA values, and bind the sole basis ID to that event. Let the engine recompute the affected complete transition input after all rate replacements in each run.
...
- Do not vary a derived probability row while leaving its rate transformation unchanged. Use only the admitted event-rate target so the engine recomputes the full transformation. Correlated rate models and transformation-space structural scenarios are not implemented.
