...
description: Derive, propagate uncertainty through, and audit AI4HEOR schema 0.5.0 cohort transition matrices or model-cycle schedules from constant cause-specific competing event rates. Use when evidence reports rates rather than cycle probabilities, each event can be bound to one extraction or proposed assumption, or uncertainty schema 0.4.0 needs rate-space DSA/PSA and an evidence-bound lognormal correlation group; do not use for probability time conversion, relative effects, general CTMCs, extrapolation, or arbitrary correlated-rate models.
---
