# AI4HEOR partitioned survival contract 0.7.0
...
Schema `0.7.0` requires analysis-plan schema `0.15.0` and exact survival-materialization, duration, cost, utility, and event-disutility artifacts. It retains the PSM `0.6.0` boundaries and adds a separately reproduced event QALY-loss schedule with an exact cross-artifact utility-overlap barrier. The portable, Python, and native validators independently check current bytes and copied values.
...
Analysis schema `0.15.0` provides the shared economic-input contract. Each strategy contains only `name`, `state_costs`, and first-cycle `state_utilities`; itemized costs reproduce annual state costs, the utility ledger reproduces state utilities, the event ledger reproduces separately excluded QALY losses, and PFS plus OS supply occupancy.
