...
2. Require a structurally complete current `heor/report-package.json`. A decision-tree report prepared from `heor-reporting/assets/decision-tree-report-package.template.json` may remain a researcher-review draft; its reproducibility package must remain `draft` and cannot become release-ready while proposed assumptions, a non-current reference case, incomplete evidence synthesis, or failed uncertainty convergence remain. Reject stale, structurally invalid, or missing deterministic results.
3. For Markov/PSM copy `assets/reproducibility-package.template.json`; for a decision tree copy `assets/decision-tree-reproducibility-package.template.json`. Write the selected template to `heor/reproducibility-package.json` only when absent. Keep it `draft` until its exact report status permits review readiness.
...
5. For Markov/PSM record the three fixed replay recipes: cost effectiveness or PSM, uncertainty, and budget impact. For a decision tree record exactly two: its base calculation and decision-tree uncertainty. Copy engine versions only from app-written results. Never substitute prose, generated code, or a different calculator.
...
7. For Markov/PSM build `source_register` from the exact union of analysis-plan and budget-impact-plan evidence sources. For a decision tree resolve every used `source_id` to the exact extraction and bibliographic record in the bound evidence synthesis. Preserve titles, types, locators, locations, verification status, and applicable hashes without fetching or copying content.
...
9. Register exactly three exhibits for Markov/PSM or exactly two for a decision tree: cost effectiveness and uncertainty only. Link each to its deterministic result and claim IDs.
10. Markov/PSM requires seven decision-facing claim entries: CHEERS 2022 items 23, 24, and 26 plus ISPOR BIA items 8, 9, 10, and 12. A decision tree requires exactly the three CHEERS entries and never invents BIA. Link each claim to bound artifacts and registered sources. Qualify limitations explicitly; never convert traceability into a truth or quality claim.
