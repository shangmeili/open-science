...
Analysis-plan schema `0.3.0` requires a `derivation` on every mapping. `derivation.model_value` is a redundant review snapshot that must equal the exact current value at `path`; changing either side invalidates the mapping.
...
Do not place a probability conversion, pooling rule, matrix assembly, calibration, interpolation, or other formula only in `selection_rationale`. This first executable contract does not evaluate free-form formulas. Keep such a mapping incomplete until a bounded deterministic adapter supports the transformation.
