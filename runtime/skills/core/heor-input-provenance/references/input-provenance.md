...
- for every schema `0.8.0` through `0.13.0` ID in `strategy_order`, or both legacy roles `comparator` and `intervention`:
  - `strategies.<strategy_id>.initial_distribution` and exactly one transition definition for schemas through `0.11.0`; neither is permitted for structure-neutral schemas `0.12.0` and `0.13.0`
...
Approvable portable analysis-plan schemas `0.3.0` through `0.13.0` require a `derivation` on every mapping. Schema `0.8.0` retains the admitted bounded transition transformations at `strategies.<strategy_id>...` paths and adds dynamic multi-strategy enumeration. Schema `0.9.0` adds background mortality; schema `0.10.0` adds bounded RR/OR application; schema `0.11.0` adds bounded constant-HR application. Schemas `0.12.0` and `0.13.0` keep only common economic inputs for partitioned survival and forbid transition transformations; `0.13.0` also binds a separately audited cost-input ledger. A transition schedule maps the complete array instead of the absent static matrix. `derivation.model_value` is a redundant review snapshot that must equal the exact current value at `path`; changing either side invalidates the mapping.
...
The schema `0.10.0` operation is exactly `relative_effect_to_transition_schedule`. Its exact fields are `operation`, `cycle_length_years`, `effect_interval_years`, `from_state_index`, `event_state_index`, `measure`, `baseline_cycle_probabilities`, `relative_effect`, and `review_bases`. Intervals are equal; `measure` is `risk_ratio` or `odds_ratio`; baseline records are ordered `{cycle, probability}` value-plus-basis objects with at least one positive value; and review bases are exactly `endpoint_alignment`, `population_transportability`, and `effect_constancy_over_cycles`. Each cycle computes `q*RR` or `q*OR/(1-q+q*OR)` and the validator independently checks the complete absorbing schedule. HR, rate ratio, risk difference, competing events, and treatment-effect extrapolation fail closed in this operation; eligible constant HR work routes to `$heor-hazard-ratio-adapter`.

The schema `0.11.0` operation is exactly `hazard_ratio_to_transition_schedule`. It accepts exactly two states, one absorbing time-to-first-event state, one cycle-aligned non-negative and non-decreasing baseline cumulative-hazard value per cycle, one positive constant HR, and exactly the `endpoint_alignment`, `population_transportability`, `proportional_hazards_assumption`, `effect_constancy_over_horizon`, and `treatment_switching_assessment` review bases. With `H0(0)=0`, every cycle computes `p=-expm1(-HR*(H0(i)-H0(i-1)))`; at least one increment is positive and the complete absorbing schedule is independently checked. Time-varying or non-proportional HRs, effect waning or stopping, unresolved switching, competing or recurrent events, curve fitting or selection, and partitioned survival fail closed.
...
Structural audit is complete only when the plan uses approvable portable schema `0.3.0` through `0.13.0`, all required paths for every declared strategy have one valid executable mapping, no mapping is duplicated or invalid, and no `unresolved` assumptions remain. Schema `0.1.0` and `0.2.0` remain calculation-only for reproducibility and cannot pass approval. Human-review readiness additionally requires the dual local-label app review of every selected extraction. This does not prove authenticated identity, source truth, population transportability, endpoint alignment, effect constancy, proportional hazards, treatment-effect duration, treatment-switching resolution, population exchangeability, absence of double counting, substantive appropriateness of a transformation, cost scope or price-basis validity, or independent duplicate extraction. Calculation may still run when incomplete, but analysis-plan approval must fail closed.
