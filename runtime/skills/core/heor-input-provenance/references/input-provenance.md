...
- for every schema `0.8.0` through `0.10.0` ID in `strategy_order`, or both legacy roles `comparator` and `intervention`:
...
Approvable portable analysis-plan schemas `0.3.0` through `0.10.0` require a `derivation` on every mapping. Schema `0.8.0` retains the admitted bounded transition transformations at `strategies.<strategy_id>...` paths and adds dynamic multi-strategy enumeration. Schema `0.9.0` adds background mortality; schema `0.10.0` adds only bounded RR/OR application. A transition schedule maps the complete array instead of the absent static matrix. `derivation.model_value` is a redundant review snapshot that must equal the exact current value at `path`; changing either side invalidates the mapping.
...
Structural audit is complete only when the plan uses approvable portable schema `0.3.0` through `0.10.0`, all required paths for every declared strategy have one valid executable mapping, no mapping is duplicated or invalid, and no `unresolved` assumptions remain. Schema `0.1.0` and `0.2.0` remain calculation-only for reproducibility and cannot pass approval. Human-review readiness additionally requires the dual local-label app review of every selected extraction. This does not prove authenticated identity, source truth, population transportability, endpoint alignment, effect constancy, population exchangeability, absence of double counting, substantive appropriateness of a transformation, or independent duplicate extraction. Calculation may still run when incomplete, but analysis-plan approval must fail closed.
