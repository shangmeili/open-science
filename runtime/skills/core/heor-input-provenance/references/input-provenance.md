...
- for every schema `0.8.0` through `0.11.0` ID in `strategy_order`, or both legacy roles `comparator` and `intervention`:
...
Approvable portable analysis-plan schemas `0.3.0` through `0.11.0` require a `derivation` on every mapping. Schema `0.8.0` retains the admitted bounded transition transformations at `strategies.<strategy_id>...` paths and adds dynamic multi-strategy enumeration. Schema `0.9.0` adds background mortality; schema `0.10.0` adds bounded RR/OR application; schema `0.11.0` adds bounded constant-HR application. A transition schedule maps the complete array instead of the absent static matrix. `derivation.model_value` is a redundant review snapshot that must equal the exact current value at `path`; changing either side invalidates the mapping.
