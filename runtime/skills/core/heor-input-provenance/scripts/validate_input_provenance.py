...
def required_paths(plan: dict[str, Any]) -> list[str]:
    ...
    structure_neutral = plan.get("schema_version") == "0.12.0"
...
def strategy_ids(plan: dict[str, Any]) -> list[str]:
    if plan.get("schema_version") in {"0.8.0", "0.9.0", "0.10.0", "0.11.0", "0.12.0"}:
...
def audit(plan: Any, synthesis: Any, synthesis_sha256: str) -> dict[str, Any]:
    ...
    if plan.get("schema_version") not in APPROVABLE_ANALYSIS_SCHEMAS:
        errors.append("schema_version must be 0.3.0 through 0.12.0 for approval review")
    ...
    if plan.get("schema_version") in {"0.8.0", "0.9.0", "0.10.0", "0.11.0", "0.12.0"}:
        ...
        if (
            ...
            errors.append(
                "schema 0.8.0 through 0.12.0 requires 2-16 unique safe strategy ids, an exact strategies object, and baseline_strategy_id first"
            )
    for role in roles:
        strategy = (plan.get("strategies") or {}).get(role) or {}
        ...
        if plan.get("schema_version") == "0.12.0" and (has_matrix or has_schedule):
            errors.append(
                f"strategies.{role} transition structure is forbidden for partitioned survival"
            )
        elif plan.get("schema_version") != "0.12.0" and has_matrix == has_schedule:
            errors.append(
