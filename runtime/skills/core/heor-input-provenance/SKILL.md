...
5. Use approvable analysis-plan schema `0.3.0` through `0.7.0`; reserve `0.5.0` for the admitted transition-rate transformation, `0.6.0` for the admitted bounded survival-curve transformation, and `0.7.0` for the admitted single-event probability-time transformation. Map every source-based path to evidence source IDs and exact `extraction_ids`. Require every selected model extraction's `extracted_value` to be strict JSON, not a narrative value plus commentary.
6. Add `derivation.method` and an exact `derivation.model_value` snapshot to every mapping. Use `direct_evidence` only when one selected extraction equals the model value, `explicit_assumption` only with no source extraction and a `proposed` assumption, `monetary_adjustment` for source-based costs or willingness-to-pay, `$heor-transition-rate-adapter` for schema `0.5.0` constant competing event rates, `$heor-survival-curve-adapter` for schema `0.6.0` already-selected two-state exponential or Weibull curves, and `$heor-probability-time-adapter` for schema `0.7.0` source probabilities with an explicit interval and at most one event per row. Do not use a free-form formula as executable evidence.
