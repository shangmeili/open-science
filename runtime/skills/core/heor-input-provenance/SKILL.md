...
5. Use approvable analysis-plan schema `0.3.0` through `0.8.0`. For schema `0.8.0`, enumerate every ID in `strategy_order` and map all required inputs for every strategy; do not cover only the baseline and one intervention. The existing bounded transition-rate, survival, and probability-time transformations remain available at dynamic strategy paths. Map every source-based path to evidence source IDs and exact `extraction_ids`. Require every selected model extraction's `extracted_value` to be strict JSON, not a narrative value plus commentary.
6. Add `derivation.method` and an exact `derivation.model_value` snapshot to every mapping. Use `direct_evidence` only when one selected extraction equals the model value, `explicit_assumption` only with no source extraction and a `proposed` assumption, and `monetary_adjustment` for source-based costs or willingness-to-pay. Use `$heor-transition-rate-adapter`, `$heor-survival-curve-adapter`, and `$heor-probability-time-adapter` for their admitted transformations in legacy schemas `0.5.0`, `0.6.0`, and `0.7.0` respectively, or at a declared strategy path in schema `0.8.0`. Do not use a free-form formula as executable evidence.
