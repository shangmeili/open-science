...
5. Use approvable analysis-plan schema `0.3.0`, `0.4.0`, or `0.5.0`; reserve `0.5.0` for an admitted deterministic transition-rate transformation. Map every source-based path to evidence source IDs and exact `extraction_ids`. Require every selected model extraction's `extracted_value` to be strict JSON, not a narrative value plus commentary.
6. Add `derivation.method` and an exact `derivation.model_value` snapshot to every mapping. Use `direct_evidence` only when one selected extraction equals the model value, `explicit_assumption` only with no source extraction and a `proposed` assumption, `monetary_adjustment` for source-based costs or willingness-to-pay, and `$heor-transition-rate-adapter` for schema `0.5.0` constant competing event rates. Do not use a free-form formula as executable evidence.
