...
5. Use approvable portable analysis-plan schema `0.3.0` through `0.9.0`. For schema `0.8.0` or `0.9.0`, enumerate every ID in `strategy_order` and map all required inputs for every strategy; do not cover only the baseline and one intervention. Existing bounded transition-rate, survival, and probability-time transformations remain available at dynamic strategy paths; schema `0.9.0` adds only background mortality. Map every source-based path to evidence source IDs and exact `extraction_ids`. Require every selected model extraction's `extracted_value` to be strict JSON, not a narrative value plus commentary.
6. Add `derivation.method` and an exact `derivation.model_value` snapshot to every mapping. Use `direct_evidence` only when one selected extraction equals the model value, `explicit_assumption` only with no source extraction and a `proposed` assumption, and `monetary_adjustment` for source-based costs or willingness-to-pay. Use `$heor-transition-rate-adapter`, `$heor-survival-curve-adapter`, `$heor-probability-time-adapter`, and `$heor-background-mortality` only for their admitted deterministic transformations. For background mortality, declare the exact life-table metadata and start age; bind each cycle's annual all-cause probability, one constant additive disease excess rate, and the `population_exchangeability` and `no_double_counting` review bases. Those review bases are evidence or proposed assumptions, never approval state. Do not use a free-form formula as executable evidence.
