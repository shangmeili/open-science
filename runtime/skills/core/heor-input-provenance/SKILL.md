...
5. Use approvable portable analysis-plan schema `0.3.0` through `0.10.0`. For schemas `0.8.0` through `0.10.0`, enumerate every declared strategy and map every required input. Schema `0.9.0` adds only background mortality; schema `0.10.0` adds only bounded RR/OR relative-effect application. Map every source-based path to exact source and extraction IDs and require strict-JSON extracted values.
