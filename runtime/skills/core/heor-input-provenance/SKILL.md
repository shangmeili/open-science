...
5. Use analysis-plan schema `0.3.0`. Map every source-based path to evidence source IDs and exact `extraction_ids`. Require every selected model extraction's `extracted_value` to be strict JSON, not a narrative value plus commentary.
