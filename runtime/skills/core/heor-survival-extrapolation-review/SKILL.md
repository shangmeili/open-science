...
1. Read the decision problem, conceptual model, evidence synthesis, analysis plan, input provenance, and data classification. Confirm the endpoint, population, curve label, time origin, event and censor definitions, time unit, observed follow-up boundary, and modeled horizon in natural language. Copy the current `analysis_id` and exact parametric-survival `input_provenance[].path` into `analysis_target`; stop if the plan has more than one such target because the alpha does not yet admit a multi-review collection.
...
9. Run `scripts/validate_survival_extrapolation_review.py heor/survival-extrapolation-review.json --workspace <project-root> --analysis-plan heor/analysis-plan.json`. Fix structural, target, hash, landmark, or completeness errors. The desktop analysis-plan approval independently repeats the audit and remains authoritative for the selected curve and downstream use.
