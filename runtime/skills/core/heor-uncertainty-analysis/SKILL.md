...
3. Select parameter targets only from the engine allowlist. Link each target to one `distribution_available` provenance mapping. Do not invent a range, distribution, or dependence assumption from a point estimate. Use uncertainty schema `0.3.0` when any target is a rate inside an admitted analysis schema `0.5.0` transformation.
...
7. State the independence rationale and resolve known omitted correlations. List PSA omissions with a reason; omission is visible, not silently converted to fixed certainty.
...
11. Write schema `0.3.0` `heor/uncertainty-plan.json` from the bundled template. Bind `base_analysis.content_sha256` to the final exact bytes of `heor/analysis-plan.json`. Schemas `0.1.0` and `0.2.0` remain readable; only `0.3.0` admits event-rate targets, while `0.1.0` evaluates only the primary threshold.
...
- Do not use beta or Dirichlet for event rates, change a derived probability row, or describe independent event-rate draws as correlated. Resolve known omitted correlations before review.
