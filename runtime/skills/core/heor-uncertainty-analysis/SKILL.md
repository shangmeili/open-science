...
3. Select parameter targets only from the engine allowlist. Link each target to one `distribution_available` provenance mapping. Do not invent a range, distribution, or dependence assumption from a point estimate. Use current uncertainty schema `0.6.0`; schemas `0.1.0` through `0.5.0` remain readable for reproducibility.
...
- Do not use transformation-parameter targets for curve fitting or selection, probability-time conversion outside the admitted single-event contract, HR/RR/OR application, pooling, general CTMC intensities, competing risks, PFS/OS combination, background mortality, clinical extrapolation validity, or transformation-space structural scenarios.
