...
6. Treat a schema `0.5.0` rate-derived transition as fixed in the current uncertainty engine. Do not target its probability row or change point while leaving the transformation snapshot unchanged; rate-space DSA/PSA and transformation-space structural scenarios require a future adapter.
...
11. Write schema `0.2.0` `heor/uncertainty-plan.json` from the bundled template. Bind `base_analysis.content_sha256` to the final exact bytes of `heor/analysis-plan.json`. Schema `0.1.0` remains readable but evaluates only the primary threshold.
