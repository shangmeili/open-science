...
- Under current analysis `0.15.0` / PSM `0.7.0`, uncertainty `0.13.0` and `0.14.0` bind the exact current PSM plan, curve materializations, treatment-effect duration, cost normalization, utility inputs, and event-disutility bytes in `partitioned_survival_inputs`. Schema `0.14.0` additionally binds the joint-survival manifest and draws. A change to any source invalidates the uncertainty review.
...
- only under analysis `0.12.0` with uncertainty `0.11.0` or `0.12.0`, exact state-cost or state-utility scalars. No transition or survival-curve parameter target is admitted; schema `0.12.0` receives complete curves only through the separate joint-draw artifact.
- only under analysis `0.15.0` / PSM `0.7.0` with uncertainty `0.13.0` or `0.14.0`, a parameter declares `artifact` and a JSON Pointer relative to that bound artifact. Cost targets are `annual_quantity.value`, `unit_price.amount`, or an adjustment `factor`; utility targets are a non-dead `source_utility.value` or one cycle-specific multiplicative adjustment factor; event targets are one occurrence/exposure schedule value, `utility_decrement`, or non-continuous `duration_days`. Derived normalized costs, annual state costs, cycle utility schedules, per-event QALY losses, aggregate event schedules, and analysis-plan reward vectors are forbidden targets.

For schema `0.13.0`, `provenance_path` is exactly `<artifact>.<JSON Pointer tokens>`. DSA bounds bracket the artifact value and remain in its domain. Uniform is admitted when its complete bounded support remains in-domain. Gamma and Lognormal are admitted only for non-negative or positive components; they are not admitted for health-state utilities, probabilities, or exposure fractions. After all replacements, engine `0.14.0` deterministically rebuilds normalized unit and annual costs, strategy/state cost totals, cycle utilities, per-event QALY loss, and strategy/cycle/state event-loss schedules before evaluating the fixed PSM curves. Any invalid support realization, one-time schedule, duration, utility bound, or combined event-loss anchor fails closed.

Schema `0.13.0` returns `calculation_classification=component_parameter_uncertainty` and `uncertainty_scope=cost_utility_event_components_only`. Every fixed PFS/OS curve remains an explicit omission; the result is not complete PSM uncertainty.

Schema `0.14.0` uses the same component allowlist and recomputation with joint-survival manifest `0.3.0`. After validating both hash-bound artifacts, engine `0.15.0` consumes the correspondingly indexed complete PFS/OS row and one component draw in every PSA iteration. Base case, DSA, discount/half-cycle scenarios, and separately reported treatment-duration alternatives retain reviewed deterministic curves. Output is `calculation_classification=joint_curve_and_component_parameter_uncertainty` with `uncertainty_scope=joint_survival_curves_and_cost_utility_event_components`.

Represented PFS/OS curves must not appear in schema `0.14.0` omissions. Curve-family selection, extrapolation assumptions, and source-model validity must remain explicit omissions. Treatment-duration alternatives remain separately deterministic rather than being model-averaged or sampled. This is composed parameter uncertainty under selected structural assumptions, not complete structural uncertainty.

The current composition preserves dependence inside each joint survival row and inside admitted component groups, but does not represent dependence between survival rows and component draws. The Human must justify cross-domain independence in `correlation_handling.independence_rationale`; known unsupported dependence remains a blocker and cannot be hidden as independence.

For partitioned survival, `target` is an exact scalar JSON Pointer while `provenance_path` is the corresponding dot-path reward vector without the final index, matching the common evidence-input contract. State costs use non-negative DSA bounds and Gamma, Lognormal, or non-negative bounded Uniform PSA. State utilities use DSA bounds inside `[-1,1]` and Beta or Uniform bounded inside `[-1,1]`.

Under uncertainty `0.11.0`, every strategy's PFS and OS curve appears in `omitted_parameters`; the engine holds those curves fixed and returns `calculation_classification=partial_parameter_uncertainty` plus `uncertainty_scope=economic_inputs_only`.

Under uncertainty `0.12.0`, `$heor-joint-survival-uncertainty` supplies exactly one complete, hash-bound PFS/OS row per PSA iteration from an admitted joint posterior or paired-patient bootstrap. Represented PFS/OS curves must not appear in `omitted_parameters`. Curve-family selection, extrapolation assumptions, and treatment-effect duration must appear as explicit structural omissions. Engine `0.13.0` returns `calculation_classification=joint_curve_draw_parameter_uncertainty` plus `uncertainty_scope=joint_survival_curves_and_economic_inputs`. It validates artifact coherence but does not prove the source posterior or bootstrap is statistically or clinically appropriate.

Both schemas produce CEAC, CEAF, convergence statistics, and per-person EVPI conditional on represented inputs and selected structural assumptions. Neither supports a release-ready complete-PSM claim by itself.
...
Dirichlet sampling preserves dependence within one probability row. Schemas `0.4.0` through `0.12.0` additionally admit bounded cross-parameter dependence only when all of the following hold. Schema-specific target restrictions can still make a group impossible; for example, uncertainty `0.8.0`, `0.9.0`, and `0.10.0` each permit only one transformed effect parameter. Schemas `0.11.0` and `0.12.0` may group only evidence-bound Lognormal economic scalars; Beta or Uniform utility inputs cannot enter such a group. Survival-curve dependence in `0.12.0` is carried by the complete joint draw row, not this correlation matrix:
...
Schemas `0.13.0` and `0.14.0` add one separate, named dependence fragment for component parameters. A group contains 2–32 Uniform or Lognormal marginals, uses `scale=latent_standard_normal` and `method=gaussian_copula_cholesky`, and supplies a finite symmetric strictly positive-definite matrix in `parameter_ids` order. Each latent correlated normal is exponentiated for a Lognormal member or passed through the standard-normal CDF and the declared Uniform quantile. The matrix is therefore a latent Gaussian-copula correlation, not an observed-scale Pearson or rank correlation. Group basis IDs must be linked by every member. AI4HEOR does not infer this matrix from common sources, convert observed correlations, or admit Gamma/Beta members, perfect correlation, arbitrary copulas, or empirical component draws in this fragment. Joint survival dependence is carried separately by the complete row.
...
- [NICE DSU TSD 6 on software for probabilistic cost-effectiveness analysis](https://www.sheffield.ac.uk/sites/default/files/2022-02/TSD6-Software.final_.08.05.12.pdf) requires the joint uncertainty structure produced by evidence synthesis to be propagated through the decision model. AI4HEOR implements the bounded legacy lognormal and schema `0.13.0` latent Gaussian-copula fragments above, not broader MCMC, coefficient-covariance reconstruction, arbitrary copulas, bootstrap, or posterior-sample workflows.
- [Ades, Claxton, and Sculpher on evidence synthesis, parameter correlation, and PSA](https://pubmed.ncbi.nlm.nih.gov/16389628/) supports propagating correlation induced by the evidence structure rather than independently sampling related inputs. AI4HEOR still requires a Human-supplied linked matrix and does not estimate it.
- [Xie et al. on cost and utility parameter correlations](https://pubmed.ncbi.nlm.nih.gov/37264680/) supports making cost/utility dependence explicit because it can change output variance and means. AI4HEOR does not implement the paper's correlation-estimation methods; it only propagates an already reviewed latent matrix.
...
- [NICE DSU TSD 19 on partitioned survival analysis](https://sheffield.ac.uk/media/34205/download?attachment=) identifies independently modelled PFS and OS as a structural assumption and cautions that independently simulated endpoints can become incoherent. Schema `0.12.0` therefore consumes one complete cross-endpoint, cross-strategy curve row per iteration and rejects independent endpoint sampling.
- The [hesim partitioned-survival vignette](https://hesim-dev.github.io/hesim/articles/psm.html) and a [Bayesian partitioned-survival model](https://pmc.ncbi.nlm.nih.gov/articles/PMC8488644/) demonstrate propagation through sampled parameter sets or joint posterior draws. AI4HEOR adopts only a backend-neutral interchange contract, not either fitting implementation.
