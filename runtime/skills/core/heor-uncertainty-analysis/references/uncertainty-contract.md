...
- Under current analysis `0.15.0` / PSM `0.7.0`, uncertainty `0.13.0` binds the exact current PSM plan, curve materializations, treatment-effect duration, cost normalization, utility inputs, and event-disutility bytes in `partitioned_survival_inputs`. A change to any source invalidates the uncertainty review.
...
Schemas `0.2.0` through `0.13.0` require `probabilistic_analysis.decision_thresholds` with a rationale and 2–101 unique, non-negative, strictly increasing values including the positive primary `willingness_to_pay`. Schema `0.1.0` remains readable and produces one primary-threshold row only. Uncertainty `0.13.0` is reserved for analysis `0.15.0` / PSM `0.7.0`; earlier schema pairings remain unchanged.
