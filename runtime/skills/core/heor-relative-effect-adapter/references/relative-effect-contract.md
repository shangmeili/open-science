...
Stop when the source supplies a hazard ratio, rate ratio, risk difference, incompatible adjusted/conditional effect, competing or recurrent event, composite endpoint, non-absorbing event, multiple origin states, time-varying effect, treatment waning, crossing hazards, treatment switching, or unsupported extrapolation. Do not silently turn HR into RR, multiply a probability by HR, or convert an OR into RR. A future `$heor-hazard-ratio-adapter` must own proportional-hazards semantics and full baseline-hazard schedule recomputation; `$heor-survival-curve-adapter` remains limited to already-selected absolute survival curves.
