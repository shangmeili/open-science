...
Translate a natural-language economic analysis scope into the common, deterministic inputs in `heor/analysis-plan.json` schema `0.13.0`. Keep survival structure and cost decomposition in their dedicated artifacts.
...
3. Copy [assets/partitioned-survival-analysis-plan.template.json](assets/partitioned-survival-analysis-plan.template.json) to `heor/analysis-plan.json`, replace every placeholder, and link `heor/cost-input-normalization.json` without embedding its hash.
...
6. Use `$heor-cost-input-normalization` to decompose every annual state-cost rate and `$heor-input-provenance` to bind every required input to evidence or an explicit proposed assumption. Do not invent or silently normalize costs, utilities, thresholds, or time bases.
...
- Costs must be finite, non-negative annual state rates exactly reproduced by the cost artifact. Utilities must be finite from -1 to 1. Arrays follow the declared state order.
