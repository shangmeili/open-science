...
- 2–16 explicitly ordered strategies under schema `0.8.0` through portable schema `0.10.0`, or legacy `comparator` and `intervention` roles under schemas `0.1.0`–`0.7.0`;
...
The relative-effect portable contract computes `p=q*RR` or `p=q*OR/(1-q+q*OR)` independently for every cycle. It requires equal cycle/effect intervals, at least one positive baseline risk, exact value-plus-basis inputs, and the exact `endpoint_alignment`, `population_transportability`, and `effect_constancy_over_cycles` review bases. Stop for HR, rate ratio, risk difference, competing events, or inferred extrapolation. The portable schema and validators do not by themselves establish native calculation support; do not approve a `0.10.0` plan until the desktop/Python/Rust implementation and parity tests are present.
