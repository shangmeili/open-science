...
10. Use `$heor-uncertainty-analysis` to create and validate `heor/uncertainty-plan.json`. Pair analysis schema `0.8.0` only with uncertainty schema `0.7.0` and its multi-strategy template; preserve schema `0.6.0` for an existing analysis schema `0.7.0` project. Bind exact plan bytes; derive ranges, distributions, correlations, convergence thresholds, and structural scenarios only from linked evidence or explicit reviewable assumptions. For admitted transformations, target the exact source rate, curve parameter, or source probability and let the engine recompute the complete transition input. Never invent a threshold, repair a correlation matrix, choose a survival curve, or vary a derived row.
...
- Map every required engine input through approvable schema `0.3.0` through `0.8.0` `input_provenance`, including an exact derivation snapshot. Schema `0.8.0` admits the existing bounded transition transformations for any declared strategy ID and adds fully incremental multi-strategy analysis. Never let external tools bypass this contract.
