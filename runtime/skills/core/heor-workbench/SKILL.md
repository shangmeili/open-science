...
7. Use `$heor-cohort-state-transition` when translating the conceptual model into static or piecewise model-cycle-dependent transitions. When evidence supplies constant cause-specific competing event rates instead of cycle probabilities, use `$heor-transition-rate-adapter`. When an exponential or Weibull all-cause curve has already been selected for an exactly two-state model, use `$heor-survival-curve-adapter`. Stop rather than fitting or selecting a curve implicitly, inventing a broader conversion, or using a schedule to imitate material time-in-state, patient history, interactions, or heterogeneity.
...
- Map every required engine input through approvable schema `0.3.0`, `0.4.0`, `0.5.0`, or `0.6.0` `input_provenance`, including an exact derivation snapshot. Reserve `0.5.0` for the admitted transition-rate transformation and `0.6.0` for the admitted bounded survival-curve transformation. Never let external tools bypass this contract.
