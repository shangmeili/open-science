...
- Map every required engine input through approvable portable schema `0.3.0` through `0.11.0`, including an exact derivation snapshot. Schema `0.9.0` adds bounded background mortality; schema `0.10.0` adds bounded RR/OR application; schema `0.11.0` adds bounded constant-HR application with exact endpoint, population, proportional-hazards, effect-duration, and switching review bases. Review bases remain evidence or proposed assumptions and never act as approval. Never let external tools bypass this contract.
