...
7. Use `$heor-cohort-state-transition` when translating the conceptual model into transitions. Use `$heor-transition-rate-adapter` for admitted constant competing rates, `$heor-survival-curve-adapter` for an already-selected exponential or Weibull curve in exactly two states, `$heor-probability-time-adapter` only for one event per row with an explicit source interval and reviewable constant-hazard assumption, and `$heor-background-mortality` only for an exactly two-state schedule that hazard-scales annual general-population probabilities to the positive model-cycle length and adds one constant excess mortality rate. Stop rather than combining competing probabilities, fitting or selecting a curve implicitly, inventing a broader conversion, or imitating material history dependence.
8. Create or update `heor/analysis-plan.json`. Use `assets/multi-strategy-analysis-plan.template.json` and schema `0.8.0` whenever two or more alternatives must enter one fully incremental comparison; use schema `0.9.0` only when the portable background-mortality contract is required; use `assets/analysis-plan.template.json` only to preserve a legacy two-role project. Read `references/analysis-plan.md` before editing. Obtain the calculation currency and price year through natural-language scoping; do not silently keep a template example. Use `$heor-input-provenance` whenever evidence is selected for model inputs, mapped, deterministically transformed, economically normalized, audited, or prepared for review.
...
10. Use `$heor-uncertainty-analysis` to create and validate `heor/uncertainty-plan.json`. Pair analysis schema `0.8.0` only with uncertainty schema `0.7.0`; pair background-mortality analysis schema `0.9.0` only with uncertainty schema `0.8.0`. Bind exact plan bytes; derive ranges, distributions, correlations, convergence thresholds, and structural scenarios only from linked evidence or explicit reviewable assumptions. For admitted transformations, target the exact source rate, curve parameter, source probability, or—under `0.9.0`—the disease excess rate. Keep the life-table probabilities fixed in the first background-mortality uncertainty contract. Never invent a threshold, repair a correlation matrix, choose a survival curve, or vary a derived row.
...
- Map every required engine input through approvable portable schema `0.3.0` through `0.9.0` `input_provenance`, including an exact derivation snapshot. Schema `0.8.0` admits the existing bounded transition transformations for any declared strategy ID and adds fully incremental multi-strategy analysis. Schema `0.9.0` adds only the bounded background-plus-excess mortality transformation; its two review bases remain evidence or proposed assumptions and never act as approval. Never let external tools bypass this contract.
