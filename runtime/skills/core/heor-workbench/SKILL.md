...
- Map every required engine input through approvable schema `0.3.0` through `0.7.0` `input_provenance`, including an exact derivation snapshot. Reserve `0.5.0` for the admitted transition-rate transformation, `0.6.0` for the admitted bounded survival-curve transformation, and `0.7.0` for the admitted single-event probability-time transformation. Never let external tools bypass this contract.
