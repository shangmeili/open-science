...
6. Create or update `heor/analysis-plan.json` from `assets/analysis-plan.template.json`. Read `references/analysis-plan.md` before editing it. Obtain the calculation currency and price year through natural-language scoping; do not silently keep the template example. Use `$heor-input-provenance` whenever evidence is selected for model inputs, mapped, economically normalized, audited, or prepared for analysis-plan review.
7. Select exactly one bundled profile from the stated decision jurisdiction, then use `$heor-reference-case` to assess every requirement and bind `heor/reference-case-assessment.json` to the plan by exact content hash. Use `NICE-PMG36-2026-current` only for England; never silently default an England analysis to a China profile or merge jurisdictions.
8. Use `$heor-uncertainty-analysis` to create and validate `heor/uncertainty-plan.json`. Bind it to the exact current analysis-plan bytes; derive ranges and distributions only from linked evidence, preserve omissions, dependence assumptions, convergence thresholds, and structural scenarios, and obtain the CEAC/CEAF threshold range from the natural-language decision context or explicit human instruction. Never invent a jurisdictional threshold.
9. Use `$heor-budget-impact` when affordability or payer expenditure is in scope. Create `heor/budget-impact-plan.json` as a separate three-year, undiscounted, two-scenario cost calculator bound to the exact analysis-plan bytes; do not derive it from discounted cost-effectiveness totals.
10. Use `$heor-model-validation` after the analysis artifacts are stable to prepare or audit `heor/model-validation.json` and local evidence. Never fill the independent reviewer's declaration or recommendation, identify Agent work as independent review, or create validation approval.
11. After all three app-written result artifacts and a current independent-validation approval exist, use `$heor-reporting` to prepare or audit the separate CHEERS 2022 cost-effectiveness matrix, ISPOR BIA matrix, report, exact result summary, disclosures, and hash-bound release package. Never edit result files, invent the release owner, score reporting quality, or create release approval.
12. Tell the researcher exactly what changed, which model and BIA inputs, reference-case requirements, uncertainty components, validation checks, and reporting items remain unsupported, and which review gate is ready for human inspection.
13. Run deterministic base-case, uncertainty, or budget impact calculations only through the workbench review panel or its documented local command. Never recreate approval state in the workspace.
14. Interpret results in the conversation with the result classification, exact input hashes, Monte Carlo or budget diagnostics, limitations, validation, reporting, and release status. Explain CEAC and CEAF separately and describe EVPI only as a per-person upper bound over uncertainty represented in the current PSA; do not infer population EVPI, EVPPI, research funding, study design, reimbursement, or policy advice.
