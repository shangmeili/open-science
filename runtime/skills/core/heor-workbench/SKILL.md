...
8. Create or update `heor/analysis-plan.json`. For partitioned survival, use `$heor-economic-inputs`, `$heor-cost-input-normalization`, and structure-neutral schema `0.13.0`; then use `$heor-partitioned-survival`, `$heor-survival-curve-materialization`, and `$heor-treatment-effect-duration` to bind itemized annual costs, source curves, and explicit sustained, immediate-stop, and log-linear-waning scenarios. Never infer duration from an HR point estimate or add an initial distribution or transition definition. For state-transition work, use `assets/multi-strategy-analysis-plan.template.json` and schema `0.8.0`, schema `0.9.0` only for background mortality, schema `0.10.0` only for the admitted RR/OR relative-effect contract, and schema `0.11.0` only for the admitted constant-HR contract; use `assets/analysis-plan.template.json` only to preserve a legacy two-role project. Read `references/analysis-plan.md` before editing. Obtain the calculation currency and price year through natural-language scoping; do not silently keep a template example. Use `$heor-input-provenance` whenever evidence is selected for model inputs, mapped, deterministically transformed, economically normalized, audited, or prepared for review.
...
10. Use `$heor-uncertainty-analysis` to create and validate `heor/uncertainty-plan.json`. Pair analysis/uncertainty `0.8.0`/`0.7.0`, background mortality `0.9.0`/`0.8.0`, relative effect `0.10.0`/`0.9.0`, and constant HR `0.11.0`/`0.10.0` exactly. Partitioned-survival analysis `0.12.0` uses economic-only uncertainty `0.11.0` when survival draws are unavailable, or uncertainty `0.12.0` with `$heor-joint-survival-uncertainty` only when a reviewed joint posterior or paired-patient bootstrap covers every strategy PFS/OS curve. Reject independent endpoint sampling; retain curve choice and extrapolation as omissions, while reporting modeled treatment-effect duration alternatives as separate deterministic structural scenarios. Under state-transition `0.10.0`, target only `relative_effect.value`; under `0.11.0`, target only `hazard_ratio.value` with strictly positive bounded Uniform support whose high reproduces a finite schedule. Bind exact plan and auxiliary artifact bytes and never invent ranges, distributions, correlations, thresholds, curve choices, effect constancy, dependence, or derived-row values.
...
- Map every required engine input through approvable portable schema `0.3.0` through `0.12.0`, including an exact derivation snapshot. Schema `0.12.0` maps only common PSM economic inputs; schemas `0.9.0` through `0.11.0` retain their bounded state-transition transformations. Review bases remain evidence or proposed assumptions and never act as approval. Never let external tools bypass this contract.
