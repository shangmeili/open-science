...
7. Use `$heor-cohort-state-transition` when translating the conceptual model into static or piecewise model-cycle-dependent transitions. Stop rather than using a schedule to imitate material time-in-state, patient history, interactions, or heterogeneity.
...
- Map every required engine input through approvable schema `0.3.0` or `0.4.0` `input_provenance`, including an exact derivation snapshot; use `0.4.0` for transition schedules, and never let external tools bypass this contract.
