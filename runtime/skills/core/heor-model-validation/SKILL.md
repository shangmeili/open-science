...
11. For partitioned-survival analysis `0.12.0` with PSM `0.4.0`, independently recompute source materializations, comparator cumulative-hazard increments, all three duration modes, the selected base-case PFS/OS rows, `PFS <= OS`, state occupancy, and scenario cost/QALY summaries. Check that evidence horizons and HR bases are explicit and shared across scenarios, not inferred from point estimates. Confirm joint PSA binds the duration artifact and treats non-base duration alternatives as separate deterministic structural scenarios. For analysis `0.13.0` with PSM `0.5.0`, additionally reproduce every cost item's adjustment product, normalized unit price, annual quantity-times-price result, and strategy/state aggregate against the exact cost-normalization bytes. Treat index, exchange-rate, tax, price-basis, and transferability choices as Human methods judgments, and keep component uncertainty plus non-annual cost structures as unresolved until dedicated contracts ship.
