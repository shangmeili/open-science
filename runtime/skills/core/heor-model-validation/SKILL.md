...
2. Read the current bound artifacts as exact bytes. For non-PSM Markov work, schema `0.1.0` binds the analysis plan, conceptual model, uncertainty plan, and budget-impact plan. For linked PSM work, schema `0.2.0` binds those four artifacts plus the PSM plan, five current PSM inputs, and the PSM, uncertainty, and budget-impact results. For decision-tree work, schema `0.3.0` binds the evidence synthesis, decision-tree plan, decision-tree uncertainty plan, and both deterministic results; it does not invent Markov or budget-impact artifacts.
3. Copy `assets/model-validation.template.json` to `heor/model-validation.json` when no report exists for Markov work. For linked PSM work, replace its schema and `model_bindings` with `assets/psm-model-bindings.template.json`. For a decision tree, use `assets/decision-tree-model-validation.template.json`. Keep it `draft` while evidence or independent-review fields are incomplete.
...
11. For dynamic budget-impact schema `0.2.0`, add an independent budget-impact `event_state_calculations` check. Reproduce both scenarios' annual opening stock, incident allocation, incident-first capacity, displacement, costs, mortality, persistence, discontinuation destination, unmet starts, closing stock, and mass balance. This verifies flow arithmetic, not epidemiological or clinical validity.
...
13. For decision-tree analysis `0.2.0`, independently verify source and proposed-assumption bindings, economic-basis equality, every terminal-path probability and payoff, strategy aggregation, pairwise incremental results, deterministic replay, uncertainty target restrictions, seeded replay, optimal-strategy probabilities, and convergence. Cover cost effectiveness only; do not create a budget-impact validation path when no budget-impact analysis exists.
14. Leave reviewer identity, independence declaration, conflict statement, independent observations, and final recommendation for the qualified independent human reviewer. Codex may structure incomplete placeholders but must not invent them.
15. Run `python3 scripts/validate_model_validation.py WORKSPACE/heor/model-validation.json WORKSPACE`. Treat `valid` as structural readiness only.
16. Ask the independent reviewer to inspect the report and use the desktop approval surface. Never write approval events.
