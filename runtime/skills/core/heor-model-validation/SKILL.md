...
11. For partitioned-survival analysis `0.12.0` with PSM `0.4.0`, independently recompute source materializations, comparator cumulative-hazard increments, all three duration modes, the selected base-case PFS/OS rows, `PFS <= OS`, state occupancy, and scenario cost/QALY summaries. Check that evidence horizons and HR bases are explicit and shared across scenarios, not inferred from point estimates. Confirm joint PSA binds the duration artifact and treats non-base duration alternatives as separate deterministic structural scenarios. For current analysis `0.15.0` with PSM `0.7.0`, additionally reproduce cost normalization, utility schedules, event losses, and every strategy/cycle/state aggregate against the exact component bytes. For uncertainty `0.13.0`, verify each target is an allowlisted raw component, every DSA/PSA run rebuilds downstream values, group bases link every member, the latent Gaussian-copula matrix is Human supplied rather than inferred, and all fixed PFS/OS curves remain explicit omissions. Treat source suitability, distribution choice, dependence, index, exchange-rate, tax, price-basis, transferability, and event additivity as Human methods judgments; non-annual costs and complete structural uncertainty remain unresolved.
