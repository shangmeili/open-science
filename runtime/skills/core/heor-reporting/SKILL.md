...
12. For partitioned-survival analysis `0.12.0` with PSM `0.4.0`, report the immutable source curves, evidence horizon, HR and basis, selected base duration scenario, all sustained/immediate-stop/log-linear-waning scenario cost and QALY summaries, and which duration alternatives remain outside the joint PSA. For analysis `0.13.0` with PSM `0.5.0`, also report the exact cost-normalization hash, ingredient count, annual quantities, compatible units, source and normalized prices, price basis, tax status, adjustment factors, evidence bases, and strategy/state aggregates. State that component-level uncertainty and event, one-time, time-varying, capital, dynamic-BIA, and societal cost structures are not represented. Do not present a base-duration PSA as probabilistic integration over structural alternatives or an aggregate cost distribution as uncertainty over the new ingredients.
