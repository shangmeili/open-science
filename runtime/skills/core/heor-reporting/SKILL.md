...
12. For partitioned-survival analysis `0.12.0` with PSM `0.4.0`, report the immutable source curves, evidence horizon, HR and basis, selected base duration scenario, all sustained/immediate-stop/log-linear-waning scenario cost and QALY summaries, and which duration alternatives remain outside the joint PSA. For current analysis `0.15.0` with PSM `0.7.0`, also report exact cost, utility, and event artifact hashes; raw ingredients and rebuilt aggregates; overlap exclusions; and Human-owned method choices. If uncertainty `0.13.0` exists, copy its `component_parameter_uncertainty` / `cost_utility_event_components_only` classification, target and correlation bases, convergence, and every fixed PFS/OS omission. Do not present it as joint survival, structural uncertainty, probabilistic integration over duration alternatives, or a release-ready PSM PSA. Event selection/additivity, one-time/time-varying/capital/dynamic-BIA/societal costs, source-model validity, and non-represented dependence remain explicit limitations.
