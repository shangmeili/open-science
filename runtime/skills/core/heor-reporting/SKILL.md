...
8. Copy numerical summaries only from the app-owned result JSON files. Keep cost-effectiveness, uncertainty, and budget-impact conclusions separate.
...
Report the package ID and hash, report-document hash, all model/result bindings, matrix gaps, disclosures, limitations, release owner, and exact next human action. State that the app verifies structure, hashes, deterministic result reproduction, reporting coverage, and local approval state—not scientific truth, journal acceptance, or policy suitability.
